...
**Metas Conversions API fra platformen** (udkast 21/9-2026 aften, `~/Downloads/udkast-meta-send/README.md`): et SELVSTÆNDIGT cron-job `meta-send-cron` (Bucket B, hvert 5. minut, tørkørsel som standard) læser `ansoegninger` og sender to `Lead`-hændelser pr. ansøgning med fbclid (`application_started` = created_at, `application_submitted` = indsendt_at; `event_id` `<id>:started`/`:submitted`), idempotent gennem sporet `meta_haendelser`. Payloaden er KUN `fbc` (`fb.1.<created_at ms>.<fbclid>` — Metas regel for server-genereret uden cookie), `external_id` (SHA-256 af ansøgnings-id) og `client_user_agent` — aldrig em/ph/ip/navn/CVR/svar (`findForbudteNoegler` afviser svaret i drift; værn `metaSend.guard`). `ansoegning-gem` gemmer browserens user agent ved «opret» KUN når fbclid er sat (`ansoegningUserAgent.ts`, fail-soft som annoncesporet). Låsen `app_config.meta_send_aktiv` (standard false) er bevisets, ikke juraens (besluttet af Jonas 21/9-2026 — ingen jurist): rigtig afsendelse kræver `dry_run: false` OG (låsen ELLER `test_event_code`). Secret: **`META_SEND_TOKEN`** (datasæt 858180112996496), læst KUN i `_shared/metaSendAfsendelse.ts` — aldrig `META_CAPI_TOKEN` (annoncehentningens nødnavn) eller `META_ADS_TOKEN` (`metaTokenAdskillelse.guard` dom 6). Fejlede afsendelser: én mail pr. døgn til `driftModtager()`.
