...
**Afmeldinger fra eWebinar til Klaviyo** (#1088, i drift 22/9-2026 kl. 13:57): en afmelding i eWebinar = **global afmelding fra e-MAILMARKEDSFØRING** i Klaviyo (`POST /api/profile-subscription-bulk-delete-jobs`, `consent: "UNSUBSCRIBED"`, **uden `list_id`**) — **ikke undertrykkelse**: en afmelding flytter SAMTYKKET, en undertrykkelse kun rækkeevnen (besluttet af Jonas 22/9; grundlaget i `~/Downloads/recon-ewebinar-afmelding.md`). Dommen `skalAfmeldes(gammel, ny)` i `_shared/webinarAfmelding.ts` får **den nye beskeds egne felter**, ikke den flettede række — fletningen lader null stå, så en besked uden `action` ville arve en ældre «Unsubscribed» og se ud som en ny afmelding hver gang. **«Én gang pr. mail» bor i SPORET**, ikke i dommen: `klaviyo_afmeldinger` er unik på `(email) WHERE udfald = 'ok'`, fordi Klaviyo dokumenterer INGEN idempotensnøgle på endepunktet (modsat `unique_id` på `/events`). **Ingen trappe, ingen opgivelse** — en marketinghændelse må opgives efter seks forsøg, en afmelding må ikke; reglen er kun «har mailen en ok-række?» med fem minutters afstand. Tre skrivere: `ewebinar-webhook` (i samme kald, efter fletningen), `klaviyo-gensend-cron` (fejlede igen) og `klaviyo-afmeld-bagud` (Bucket B, tørkørsel + låsen `app_config.klaviyo_afmeld_aktiv`). **En afmeldt får INGEN fremmøde-hændelser** (`erAfmeldt` som port foran `byggFremmoede`), og `klaviyo-profil-cron` udelader afmeldte fra sit udvalg. **Egen secret `KLAVIYO_AFMELD_KEY`** (profiles/list/subscriptions), læst KUN i `_shared/klaviyoAfsendelse.ts:afmeldHvisNoegle` — aldrig `KLAVIYO_API_KEY`, som ikke kan få scopet tilføjet («you cannot add a scope to an existing private key»). Migration `20260922060000`, kørt i prod 22/9 13:26. **Webhook-grenen er kode, ikke drift** — den bevises af den første rigtige Unsubscribed, og beviset er en række i `klaviyo_afmeldinger` med `kilde = 'webhook'`.
...
**Metas Conversions API fra platformen** (udkast 21/9-2026 aften, `~/Downloads/udkast-meta-send/README.md`): et SELVSTÆNDIGT cron-job `meta-send-cron` (Bucket B, hvert 5. minut, tørkørsel som standard) læser `ansoegninger` og sender to `Lead`-hændelser pr. ansøgning med fbclid (`application_started` = created_at, `application_submitted` = indsendt_at; `event_id` `<id>:started`/`:submitted`), idempotent gennem sporet `meta_haendelser`. Payloaden er `fbc`, `external_id` (SHA-256 af ansøgnings-id) og `client_user_agent` (`findForbudteNoegler` afviser svaret i drift; værn `metaSend.guard`). **UDVIDET 22/9-2026 (`~/Downloads/udkast-meta-udvidelse/README.md`):** kandidaten er ENHVER ansøgning i 7-dagesvinduet med user agent og landing — klik-id'et er IKKE længere adgangsbetingelsen (webinarvejen bærer intet `fbclid`). `user_data` bærer nu også `em`, `ph`, `fn`, `ln` og `country` som **SHA-256-aftryk**, normaliseret efter Metas egne regler og KUN for de felter, ansøgningen HAR — et tomt eller uhashet felt sendes aldrig, og `HASHEDE_NOEGLER` kræver 64 hex, mens værnet afviser en rå e-mail eller et rået tal hvor som helst i payloaden. **Aldrig IP, CVR, svar eller en værdi i klartekst.** Metas egne cookier `_fbp`/`_fbc` læses af fladen ved mount (samme parser som GA, `laesMetaCookies`) og gemmes i `fbp`/`fbc_cookie`; URL'ens `fbclid` har forrang på `fbc`, ellers sendes cookien ORDRET. Ansøgeren kan bede sig fri: `ansoegninger.meta_fravalg` = true → sprunget over med grunden «fravalgt», før alt andet i dommen (migration `20260922040000`). `ansoegning-gem` gemmer browserens user agent ved «opret» for ALLE (`ansoegningUserAgent.ts`, fail-soft som annoncesporet). **TRIN 2 (22/9-2026, `~/Downloads/udkast-meta-trin2/README.md`):** tre hændelser mere, alle på ansøgningen og i SAMME cron — «Kvalificeret» (custom event, rådgiverens første `tal_med_dem`), **Schedule** (første `book`) og **Purchase** (første `company_perioder`-række med art `indgang`; `value` = `beloeb_oere` i KRONER, `currency` = `META_VALUTA` = «DKK», UMÅLT — valutaen bor på Stripe-priserne bag `lookup_key`). De tre er **IKKE website-hændelser**: de sker i vores eget system, så de bærer `action_source: "system_generated"` + `custom_data.event_source: "crm"` + `lead_event_source` (Metas CRM-integration) og HVERKEN `client_user_agent` ELLER `event_source_url` — dommen kræver dem kun af `started`/`submitted`. **fbc har nu tre led:** URL'ens `fbclid` → `_fbc`-cookien ordret → **webinartilmeldingens `fbclid`** (seneste tilmelding på samme `lower(email)` FØR ansøgningen, højst 90 dage før — Metas eneste dokumenterede levetid; tidspunktet er tilmeldingens `registreret_at`). **Kandidatreglen er omskrevet:** en ansøgning er kandidat, når NOGET er sket i vinduet (oprettelse/indsendelse, en beslutning eller en betaling) — en Purchase sker 30–60 dage efter ansøgningen og ville ellers aldrig blive fundet. Sporet udvides med tre `art`-værdier (migration `20260922050000`), ikke med en tabel. **Webinarhændelser BYGGES IKKE** (bevidst fravalg, målt i prod 21/9 22:12): eWebinars `raa` bærer ingen user agent, og en website-hændelse uden kan vi ikke kalde noget andet. Låsen `app_config.meta_send_aktiv` (standard false) er bevisets, ikke juraens (besluttet af Jonas 21/9-2026 — ingen jurist): rigtig afsendelse kræver `dry_run: false` OG (låsen ELLER `test_event_code`). Secret: **`META_SEND_TOKEN`** (datasæt 858180112996496), læst KUN i `_shared/metaSendAfsendelse.ts` — aldrig `META_CAPI_TOKEN` (annoncehentningens nødnavn) eller `META_ADS_TOKEN` (`metaTokenAdskillelse.guard` dom 6). Fejlede afsendelser: én mail pr. døgn til `driftModtager()`.
...
**/webinar delt med eksterne** (udkast 21/9-2026, `~/Downloads/udkast-webinar-deling/README.md`): et privat link pr. modtager (`webinar_delinger`: navn, udløb 90 dage, kan forlænges/lukkes) åbner `/delt/webinar?t=…` — ALT på /webinar, skrivebeskyttet, ingen skal, ingen konto. **Ingen rå rækker til den eksternes browser:** `webinar-delt` (`verify_jwt = false`, prædikat `verifyDelingstoken` FØRST) henter med service role, regner med de SPEJLEDE domme `_shared/webinarDashboard.ts` og `_shared/annoncepriser.ts` (paritet med `src/lib/webinar/`) og svarer kun med det færdige (`bygDeltSvar` → `udenRaekker`; `findForbudteNoegler` afviser svaret ellers). **Tokenet gemmes KUN som SHA-256-aftryk** (nyt i huset; begrundelsen i `_shared/delingstokenAuth.ts`), sammenlignes i konstant tid, vises én gang ved oprettelsen (`webinar-deling`, Bucket A, rådgiver-gate). Ukendt/udløbet/lukket = ét svar (403); for en kendt deling står grunden i `webinar_deling_spor` (append-only, `deling_id NOT NULL`, hver visning med ip/user-agent; DELETE kun som cascade, som `protect_aftale_spor`) — et ukendt token når aldrig sporet (kun loggen: sporet kan ikke slettes direkte, og der er ingen rate-limit). Opbevaring: en deling og dens spor slettes 12 måneder efter lukning/udløb af cron-jobbet `webinar-delinger-opbevaring` (ren SQL, migration `20260922021000`). Fladen: `WebinarVisning`/`AnnoncepriserVisning` tegner et færdigt dom fra enten hooken (rådgiver) eller functionen (ekstern). **I DRIFT 21/9-2026 21:15–21:30** (#1074 `c91fb947`; migrationerne `20260922020000` og `20260922021000` kørt 21:15, job 570; «Successfully deployed edge functions: webinar-deling, webinar-delt, ga-send-cron»). **Beviset, ét stræk:** oprettet 21:20 → vist i privat vindue 21:20 → vist igen 21:28 målt serverside (pg_net, kald 13491: status 200, 59.122 tegn, **0 forekomster af «@»** — ingen persondata over ledningen) → lukket 21:29 → `afvist_lukket` 21:29. **To forsøg med et forkert token (kald 13486 og 13489, begge 403 «ukendt») står IKKE i `webinar_deling_spor`** — det er beviset for, at et ukendt token aldrig når sporet; kun functionens log bærer dem. **UDVIDET (#1080, `e88c881c`, 22/9 ~00:05 — ønsket af Jonas 21/9 23:45):** under den store boks viser `/webinar` op til **tre mindre bokse**, én pr. programsat session EFTER den næste: antal tilmeldte (unikke mails på præcis den session), tidspunkt, «om N dage» og titel — ingen graf, og en tom liste tegner ingen række. Beregningen `kommendeEfterNaeste` og grænsen `KOMMENDE_EFTER_NAESTE_MAKS = 3` bor i DOMMEN i begge spejle (`src/lib/webinar/dashboard.ts` og `_shared/webinarDashboard.ts`) og lægges i feltet `naeste.efterfoelgende`, som **aldrig bærer rækker** — derfor går boksene også ud gennem delingen uden persondata. Værn: `webinarFlade.guard` dom 12. Udrullet: `webinar-delt` («Successfully deployed edge functions: webinar-delt») og Update; set af Jonas på `/webinar` — **én boks**, fordi kun én session efter 22/9 er programsat.

**Klokkerne som mail** (udkast 21/9-2026, `~/Downloads/udkast-klokke-mail/README.md`; besluttet af Jonas 22/9 ~15:15): rådgivernes klokker (`advisor_notifications`) forlod aldrig browseren — `send-notification-email` læser kun `notifications`, og `ADVISOR_EMAIL_DISABLED` gælder medlemmernes motor (rørt IKKE). `klokke-mail-cron` (Bucket B, hvert kvarter på :04/:19/:34/:49, tørkørsel som standard) sender tre mails ud fra typelisterne i `_shared/klokkeMail.ts` — det ENESTE sted, typerne står: ALARM (`traek_fejlet`, `invitation_fejlet`, `ansoegning_underskrevet`, `mail_spaerret`, `fornyelse_dublet`, `drift`) kun til `driftModtager()` straks; COMMUNITY (`community_opslag`) til hver rådgiver straks; MORGEN (resten) til hver rådgiver i første kørsel efter kl. 07 dansk PÅ EN HVERDAG (hverdage.ts' regel: mandag–fredag minus danske helligdage og husets tre lukkedage; weekend/helligdag venter til næste hverdag; teksten siger «siden fredag kl. 07», ikke «i går») med klokkerne fra før kl. 07. ALDRIG: `ansoegning_ny` (motoren mailer selv), `ansoegning_afholdt` (rykkeren dag 2), `community_svar` (Jonas 21/9: klokke, ikke mail — `notify-community-svar` ringer rådgivernes klokke ved et medlems svar, `communitySvarBesked.ts`, spejl af opslagets; én ULÆST klokke pr. tråd pr. rådgiver via writerens valgfri `dedupKunUlaeste`, som ingen anden kalder bruger), legacy-typerne (rækker uden `advisor_id` — Slack), og en `drift`-klokke med `reference_type` i `SELVMAILENDE_REFERENCER` (`klaviyo_haendelser`, `klaviyo_profil`, `meta_haendelser`, `ga_haendelser` — gensenderen, profil-cronen, meta-send og ga-send mailer selv: én alarm, én mail; en ny selvmailende alarm skal på listen, ellers fælder værnet). Kun EGNE rækker (`advisor_id`), aldrig en læst (`read_at`), højst én gang (`mailet_at`, sat EFTER afsendelsen; migration `20260922070000`, cron-jobbet `20260922071000`). Kildeværnet `klokkeMail.guard` fælder en ny klokke-type uden plads i en liste. **Driftsalarmer går til `driftModtager()`** (`_shared/driftModtager.ts`, ét sted) — også gensenderens og profil-cronens alarm; `raadgiverModtager` er ansøgningskøens rådgiveradresse og skifter til kontakt@ 19/10.
