...
| næste skridt på rådgiverfladen — ÉN kilde til tallene | **AdvisorDashboards `queryFn` regner MoM og nøgletal ud af `financial_reports`** (T15/T16/B2), mens NoegletalView og virksomhedssiden bruger `financial_report_facts` gennem `useCompanyFacts` og den rene, testede `trendMoM.ts`. **To kilder til de samme tal** — samme slags dublet som de to domme der blev fjernet 3/9, bare et lag længere nede. **Forsiden skal flyttes til facts FØR virksomhedssiden bygges**, ellers arver den en tredje variant. Det er en dataflytning der ændrer tal på en flade der bruges hver morgen; den tages som **egen opgave med måling før og efter**, ikke som en del af en fladebygning. **Genbrug, målt 3/9 sen aften** (`~/Downloads/recon-dashboard-queryfn.md`, uden for repoet — genskabes hvis den bruges): **én fælles datamotor kan IKKE bygges** — alle fjorten hentninger i `queryFn` er porteføljebrede, ingen tager et `company_id`. Det er FORMERNE der kan genbruges pr. virksomhed (pulse, kf-serie og MoM, `reflectionStatus`, `isNewMember`/`expiresAt`, `needsAttention`, og konstruktionen af `VirksomhedsInput`), ikke hentningerne. For én virksomhed findes mønstrene allerede: `useCompanyFacts(companyId)`, `conversations` med `.eq("company_id", …)`, milestones og handouts company-nøglet i BoardroomView. To ting `queryFn` IKKE henter, som blok 1 og 7 kræver: `budget_targets` og `agent_proposals`. **MÅLT I PROD 3/9 kl. 23:56** (SQL editor, begge veje regnet og stillet op mod hinanden pr. virksomhed og periode; `~/Downloads/recon-to-kilder.md`, uden for repoet — genskabes hvis den bruges): forsidens vej (`financial_reports`, status processed, ikke slettede, seneste pr. periode) giver **151 punkter over 20 virksomheder**; facts-vejen (`financial_report_facts`) giver **314 punkter over 21 virksomheder — heraf 144 med `data_basis = 'estimated'`**. **INGEN forskel i tallene:** for hver periode hvor BEGGE kilder har en værdi, er de enige; sammenligningen fandt nul uenigheder. Tretten rapporter har ingen matchende fact-periode — de fleste er årsrapporter, hvor omsætningen i øvrigt er NULL, og som bliver til estimerede månedspunkter i facts under en anden periodenøgle; én skiller sig ud: **Brick Works ApS, «April 2026», 1.349.013 kr.** — en almindelig månedsrapport der ikke er committet til facts. **KONKLUSION: flytningen er ufarlig for tallene selv.** Ingen virksomhed får et ANDET tal for en periode den allerede har. Det der ændrer sig er hvilke PERIODER der findes: forsiden vil begynde at bruge estimater fra årsrapporter og baselines (144 punkter) som den ikke har i dag, og holde op med at bruge rapporter der aldrig er blevet committet. Begge dele er forbedringer — estimaterne er tal nogen har godkendt, ikke-committede rapporter er tal ingen har godkendt. **BETINGELSE i flytningen:** når forsiden læser facts, skal den forholde sig til `data_basis`. Et `'estimated'` punkt må ikke uden videre udløse «omsætningen faldt 16 % MoM» — at sammenligne en estimeret måned med en målt er ikke det samme som to målte. NoegletalView har allerede `momErGyldig`, der kræver at BEGGE punkter er `'measured'`; den regel skal følge med. Reconen (§7) fandt desuden to guards der kræver at alle læsere af facts filtrerer på `data_basis` eller bærer en begrundet markør — AdvisorDashboard bærer markøren for aktivitetsfeedet, ikke for nøgletal, fordi de i dag kommer fra rapporter hvor ingen guard gælder. **FUND der skal stå tydeligt: motoren (#589) får ALLEREDE to varianter af sit input** — på forsiden bygges `FactPunkt` af rapporter, på MemberDetail af facts. Dommen blev samlet ét sted 3/9, men fodres med to forskellige sandheder. Det er derfor flytningen skal ske FØR virksomhedssiden bygges. Reconen gennemgik fjorten scenarier, hvoraf tolv kunne give forskellige tal i teorien — målingen viser at ingen af dem gør det i praksis i dag. | `docs/raadgiverfladen-design.md` §4 blok 1 og 5, §10; `~/Downloads/recon-to-kilder.md` |
