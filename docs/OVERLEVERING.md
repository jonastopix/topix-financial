...
> **ALLERSIDST — PR 1: KØENS UDLØBET-REGEL OG TØRKØRSEL (§15); RETTELSER TIL §14.** Chattens beslutninger på Jonas' krav: en række til en bruger hvis virksomheder ALLE er tier expired stemples uden mail (grund «udloebet», `_shared/mailModtager.ts`, importerer `computeMembershipTier`), før chat-grupperingen og udvælgelsen, for alle typer; tørkørsel med body `{"dry_run": true}` (+ `nu`) sender og stempler intet og siger hvad der ville ske — alle sideeffekter bag `send()` og `stempl()`, låst af nyt kildeværn. dagskvote- og mailFejl-værnene urørte og grønne; `mailFejl.guard:81-90` kun omdøbt (legacy-DDL uden CHECK, prod har). tsc 0; 3521 tests (232 filer); deno grøn på motoren, index.ts kan ikke checkes lokalt (npm); edge-auth 68/68. IKKE udrullet: functionen OG `mailModtager.ts` eksplicit i build-chatten, derefter tørkørsel fra SQL editoren. **Rettelser til §14 (mærket):** rapportpåmindelser rammer IKKE udløbne (recon-fornyer tog fejl; chatten gav det videre uden efterprøvning); /virksomheder FINDER udløbne ved søgning (bevist på skærm 21:54, Jonas: «Det virker jo faktisk fint.»); ingen mails er gået til CARMA/Studio Mini siden slutdatoen (målt 21:49). Camilla Risager SKREVET 21:55 (DEL 3-rækken LUKKET). DE TYVE (6): F kan ikke siges til de nye (webinaret er optaget) → nyt kort «De nye skal starte med juni–august — det skal stå i platformen»; I og J som DEL 3-rækker. Mangellisten: 165 kort, 16 lukkede.
> **NU — PR 1 UDRULLET, PR 2 BYGGET (§15, §16).** PR 1: #896 (`8216ad61`) merget 20:19:14 UTC, udrullet 20:21 UTC med `mailModtager.ts` (Lovable ordret i §15); tørkørsel fra SQL editoren 20:23 svarede 200 med `dry_run: true`, `udloebet: 0` — den nye kode kører, reglen er UBEVIST i drift (tom kø; bevis = `udloebet ≥ 1` og ingen mail, tilføjet «Beviser der udestår»). PR 2 (chattens beslutninger 1–6): `event_published` og `community_opslag` er UDE af køens første hentning (ellers fylder de alle 50 pladser og stopper chatsvar hele dagen) og hentes i egen forespørgsel kun i vinduet 17–20 dansk, loft 500; udløbet-reglen på dem; `fordelSamlemail` → `bygSamlemail` → `send()` med label `notification-samlemail`; løkken efter de to eksisterende og gated af `rateLimit`; svaret bærer `samlemail: {…}`. Værnene ændret bevidst: `skalKoeStoppe` 2 → 3 i dagskvote.test og mailFejl.guard (ordret i §16), tredje løkke som eget udsnit; dryrun-værnet holder; nyt samlemail-værn (8). tsc 0; 3529 tests (233 filer); deno grøn på begge motorer; edge-auth 68/68. IKKE udrullet: functionen OG `_shared/samlemail.ts` eksplicit i build-chatten, tørkørsel med `nu` kl. 17:30 dansk, bevis senest 21/9 — ellers fallback. Efter udrulningen samles events og opslag til kl. 17.
> **SENEST I NAT — PR 2 UDRULLET, PR 3 BYGGET, STUDIO MINI LUKKET (§16, §17).** PR 2: #897 (`bab6848a`) merget 20:40:59 UTC, udrullet 20:43 UTC med begge delte filer (Lovable ordret i §16); tørkørsel 20:44 dansk med `nu` = 16/9 kl. 17:30 → 200, `samlemail.i_vinduet: true`, hentet 0, kontroltalt 0 i SQL — koden kører, filteret og vinduet virker; fordeling, mail og stempling UBEVIST (bevis = første rigtige samlemail 16/9 kl. 17 dansk, senest 21/9, ellers fallback; tilføjet «Beviser der udestår»). Fra nu af samles events og opslag til kl. 17. **PR 3 (vinduesmailene, Jonas' valg A; chattens C1–C5):** motoren svarer `"vindue_1"` (dag 1–10) og `"vindue_2"` (dag 11–14) for `udloebet_tilbyd` med de rigtige abonnementsfelter; byggerne, cronen, migrationen `20260915230000` (ikke kørt); tre eksisterende tests ændret bevidst (ordret i §17); tsc 0; 3552 tests (233 filer); deno grøn på de to `_shared`-filer, cronen kan ikke checkes lokalt (npm); edge-auth 68/68. **TEKSTERNE STÅR ORDRET I §17 TIL JONAS' GODKENDELSE FØR UDRULNING**; rækkefølgen (godkend → merge → migration → udrul cron + `_shared` → tørkørsel med CARMA vindue_1 → kl. 11 UTC sender) står dér. **Studio Mini — DE TYVE (4) LUKKET:** Jonas valgte «tilbyd ikke» 22:29 UTC med noten «Forlænger ikke (Jonas 6/9)»; CARMA er nu den eneste i vinduet med tilbyd (vindue 1 ved første kørsel efter udrulning, vindue 2 den 22/9). Mangellisten: Studio Mini-kortet lukket (bliver stående); 165 kort, 17 lukkede.
> **AFTENENS REST 22:45–23:25 (§17, §18).** PR 3: teksterne godkendt (vindue 2 uden «skriv til os»); #898 (`c80c8d1b`) merget 21:17:00 UTC, CI grøn; migrationen `20260915230000` KØRT 23:11 dansk FØR commit (begge kolonner målt); UDRULNINGEN VENTER — Lovable i vedligehold ca. 23:20 («Waiting for the backend maintenance window to end before retrying»), SQL editoren kunne heller ikke køre tørkørslen; HÅRD FRIST: cron + `fornyelsesvarsel.ts` + `fornyelsesMail.ts` udrullet før kørslen kl. 11 UTC 22/9 (CARMA's dag 11; vindue 1 kan gå t.o.m. 21/9). DE TYVE (9) skrevet om: Din Forsikringsret (Monday-vejen, 15:34 UTC) FIK website fra «Hjemmeside» — det der mangler er branchen (Monday-kolonnen tom, CVR-opslaget fejlede tavst); tjeklisten «1 af 8». CVR-opslaget: cvrapi anonymt, ingen timeout, null ved fejl (`virksomhedsOprettelse.ts:64-102`); berigelsen kørt 23:05 — cvrapi svarede ikke fra edge-runtimen, intet skrevet; nye ad Monday-vejen vil sandsynligvis mangle branchen fra start. (16) Virk: ansøgning sendt 23:13 til cvrselvbetjening@erst.dk, svar afventes. (13) Monday-oprydningen: betingelsen opfyldt (Jonas: «Godkendt i mandag og den blev grebet af platformen 100% og der er betalt»), klar til oprydning, ikke gjort. Mangellisten: n14-12 omskrevet, n14-6a og n14-7 ført à jour; 165 kort, 17 lukkede.
> **FUND K RETTET I KODE (§19).** Jonas' valg A ordret: titel «The Boardroom — sparring for virksomhedsejere», beskrivelse «12 måneder med sparring fra Morten Larsen og Jonas Herlev og et netværk af andre selvstændige. Medlemskab efter ansøgning på theboardroom.dk.» — i `index.html` (title, description, og:, twitter:) og `public/manifest.json`; robots noindex, canonical, og:url/type/image og twitter:card/image uændrede; nyt kildeværn `linkPreview.guard.test.ts`; tsc 0; 3557 tests (234 filer). Ingen andre forekomster af den gamle tekst i `src/`/`public/`. Delingen udleverer aldrig app-linket (recon), men app'en er en SPA, så ethvert app-link viser tags'ene. Frontend — kræver Update-klik; BEVIS = Post Inspector på app.theboardroom.dk viser den nye tekst; ubevist til da. Kort w-k: rettet i kode, står åbent til beviset; ny DEL 3-række (der fandtes ingen).
>
...
**15. KØENS UDLØBET-REGEL OG TØRKØRSEL — PR 1 (15/9 sent; bygget, testet — #896 merget 20:19:14 UTC, UDRULLET 20:21 UTC, tørkørsel 200 kl. 20:23; reglen UBEVIST i drift).**

- **CHATTENS beslutninger 15/9 (ikke Jonas'), på Jonas' krav «ingen platformsmails i forlængelsesvinduet» (§14):** **1) Reglen:** en notifikationsrække til en bruger, hvis virksomheder (`company_members` → `companies`) ALLE har tier «expired» efter `computeMembershipTier`, stemples uden mail med grund «udloebet». Én ikke-udløbet virksomhed er nok til at få mail (som `har_aktivt_medlemskab`'s EXISTS). En bruger uden `company_members`-række (rådgivere m.fl.) er uberørt. Reglen gælder ALLE typer køen henter, også chat-samlingen, og afgøres FØR chat-grupperingen og `selectNotificationEmails`. **2) Tørkørsel:** body `{"dry_run": true}` → ingen `sendManagedEmail`, ingen stempling, ingen `email_send_log`; svaret bærer `dry_run: true`, `nu`, og lister (id, user_id, type) for hvad der VILLE sendes og hvad der VILLE stemples med grund. Tom eller ugyldig body (cronens `{}`) → sender som i dag. `nu` (ISO) læses KUN når `dry_run` er true; fejler body-læsningen, er det IKKE tørkørsel.
- **Fra `recon-koe-regel-og-vinduesmails.md` (vindue B):** en bruger kan være i flere virksomheder (`company_members` UNIQUE(company_id, user_id), ingen UNIQUE på user_id); `user_company_id` er LIMIT 1 uden ORDER BY — derfor går koblingen user_id → alle `company_members`, ikke via den; TS- og SQL-dommene har samme grænse (expired fra 00:00 UTC dagen efter slutdatoen); `send-notification-email` havde ingen tørkørsel før (læste aldrig body); vinduesmailene mangler stempelkolonner (`company_fornyelse` har præcis to, «30 dage før»/«7 dage før»), og data skelner ikke «sagde nej» fra «intet svar» (`beslutning` er kun `tilbyd | tilbyd_ikke`).
- **Motoren `_shared/mailModtager.ts`** (ren, ingen I/O; importerer `computeMembershipTier` fra `./membershipTier.ts`, ingen kopi): `erAlleVirksomhederUdloebet(virksomheder, nu)` — false for tom liste, true kun når ALLE er expired; `fordelUdloebne(raekker, medlemskaber, nu)` → `{ udloebne, resten }` med invarianten hver række præcis ét sted; `UDLOEBET_GRUND = "udloebet"`. Tests i `src/lib/__tests__/mailModtager.test.ts` (9): tom liste; én aktiv; én udløbet; udløbet + aktiv; no_date; subscriber; CARMA-grænsen (slut 2026-09-11: 11/9 23:59:59Z ikke udløbet, 12/9 00:00:00Z udløbet); bruger uden medlemskab; invarianten.
- **Integrationen i `send-notification-email/index.ts`:** body-læsningen lige efter klienten (`req.text()` i try/catch, tom body = `{}`; `toerKoersel` kun ved `dry_run === true`; `nu` kun dér). De to hjælpere defineres EFTER hentningen af kandidaterne (så første `.from("notifications")` stadig er hentningen — dagskvote-værnets anker) og før `userIds`: `stempl(n, grund)` er det ENESTE sted med `.update({ email_sent_at …` og `send(args, raekker)` det ENESTE `sendManagedEmail(`-kald; i tørkørsel registrerer de i `ville_stemple`/`ville_sende` (sendte rækker står kun i `ville_sende`). Udløbet-reglen kører lige efter `userIds`: `company_members` for kandidaternes user_ids, `companies (id, contract_end_date, subscription_status, subscription_current_period_end)`, `fordelUdloebne(pending, medlemskaber, nu)`; de udløbne stemples via `stempl(notif, UDLOEBET_GRUND)` med log «[dispose] IKKE SENDT — udloebet: …», tælles i `udloebet` og `skipped`, og `raekkerTilMail` (resten) går til chat-grupperingen og videre. Alle ni tidligere stemplinger (dispose-løkken, chat læst, advisor ×2, pref ×2, tråd væk, sendt ×2) går nu gennem `stempl` med grund; begge afsendelser gennem `send`. Skabelon-auto-oprettelsen (`email_templates.insert`) springes også over i tørkørsel. Svaret får `udloebet`, og i tørkørsel `dry_run`, `nu`, `ville_sende`, `ville_stemple`. `nu` styrer i PR 1 kun udløbet-reglen — `fifteenMinAgo`, dagskvotens «i dag» og `selectNotificationEmails(candidates)` er uændrede (det sidste er låst ordret af dagskvote-værnet).
- **Værnene:** `dagskvote.test.ts:91-125` og `mailFejl.guard.test.ts:37-64` er IKKE ændret og er grønne — alle tællinger (to kvote-gates, to `advisorUserIds.has(`, to `if (!userEmail) {`, to `skalKoeStoppe`-løkker, `if (rateLimit) break;`) og ankre holder, fordi reglen ligger før chat-grupperingen og hjælperne efter hentningen. Nyt kildeværn `src/lib/__tests__/sendNotificationEmail.dryrun.guard.test.ts` (7): præcis ét `sendManagedEmail(` og præcis ét `.update({ email_sent_at`/`email_sent_at:` i index.ts, begge inde i hjælperne og efter `if (toerKoersel)`; hjælperne efter hentningen; body-læsningen sætter kun tørkørsel ved `=== true` og aldrig ved `!== false`; `nu` læses kun i dry_run-blokken; `fordelUdloebne(` før `for (const notif of raekkerTilMail)` og før `selectNotificationEmails(candidates)`; svaret bærer tørkørselsfelterne kun i tørkørsel. **`mailFejl.guard.test.ts:81-90` omdøbt og kommenteret** — kun navn og kommentar, samme assertions. Var: `describe("mailFejl.guard — loggens status er kendt af fladen uden migration"` / `it("EmailLogView kender rate_limited som status og filter; email_send_log.status har ingen CHECK"`. Nu: `describe("mailFejl.guard — loggens status er kendt af fladen; legacy-DDL'en har ingen CHECK, prod har (migration 20260915210000)"` / `it("EmailLogView kender rate_limited som status og filter; legacy-DDL'en 20260226224654 har ingen CHECK på status (prod HAR email_send_log_status_check)"`, med kommentaren «RETTET 15/9: dette værn læser LEGACY-DDL'en 20260226224654 (CREATE TABLE uden CHECK) — men prod HAR email_send_log_status_check (målt 15/9), og rate_limited er tilladt dér af migrationen 20260915210000 (kørt 15/9 21:14 dansk). Assertions er uændrede; kun navn og kommentar siger sandheden. Prod-reglen vogtes af emailSendLogStatus.guard.test.ts.»
- **Målt:** `bunx tsc --noEmit -p tsconfig.app.json` exit 0; `bun run test`: Test Files 232 passed (232), Tests 3521 passed (3521); `deno check supabase/functions/_shared/mailModtager.ts` grøn; `bun run check:edge-auth`: «PASS — all 68 triggered files contain at least one auth predicate.» Husets eneste typecheck af edge functions er edge-auth-værnet (workflows: `test.yml` kører `tsc -p tsconfig.app.json` + `bun run test`, `edge-function-auth.yml` kører `check-edge-function-auth.ts`; ingen `deno check` i CI). `deno check supabase/functions/send-notification-email/index.ts` kan ikke køres lokalt, ordret: «error: Could not find a matching package for 'npm:@lovable.dev/email-js@0.1.0' in the node_modules directory. Ensure you have all your JSR and npm dependencies listed in your deno.json or package.json, then run `deno install`. Alternatively, turn on auto-install by specifying "nodeModulesDir": "auto" in your deno.json file. at file:///…/supabase/functions/_shared/managedEmail.ts:15:49» — DEL 4 «`deno check` lokalt kræver npm-opløsning»; index.ts' egne typer er derfor kun læst, ikke checket, før udrulningen (Lovables build siger det).
- **Næste skridt:** (1) udrulning — `send-notification-email` OG den nye delte fil `_shared/mailModtager.ts` → eksplicit i build-chatten (en ændret/ny `_shared`-fil udrulles ikke af sig selv, DEL 4), og læs `git pull`-outputtet bagefter (§11); (2) tørkørsel fra SQL editoren med `net.http_post`-mønstret (recon-koe-regel §6: url `…/functions/v1/send-notification-email`, vault-nøglen `email_queue_service_role_key`, `timeout_milliseconds := 150000`, svaret i `net._http_response`) og body `{"dry_run": true}` — og med `"nu"` sat til fx CARMA's dag 12/9 for at se reglen; (3) PR 2 = samlemailens integration (§12); (4) PR 3 = vinduesmails dag 1 og 11 (§14, Jonas' valg A) — kræver nye stempelkolonner og en ændring af gren 4 i `fornyelsesvarsel.ts`, låst af `fornyelsesvarsel.test.ts:163-171, :231` og pariteten.
- **UDRULLET 15/9 kl. 20:21 UTC** i Lovables build-chat, ordret: «Udrulningen af `send-notification-email` lykkedes kl. 2026-09-15 20:21 UTC fra main (commit 8216ad61, PR #896), med den delte fil `_shared/mailModtager.ts` med i udrulningen. Funktionen svarer live med HTTP 403 «Forbidden» ved kald uden service-role-token — forventet adgangsdom. Ingen filer, andre funktioner eller migrationer blev rørt.» `git fetch` + `git log 8216ad61..origin/main` bagefter: ingen commits (§11-lærdommen). #896 = `8216ad61`, merget 20:19:14 UTC. **TØRKØRSEL fra SQL editoren 20:23:20 UTC** (`net.http_post`, body `{"dry_run": true}`), request_id 10829, svar ordret: status 200, `{"processed":0,"sent":0,"skipped":0,"udloebet":0,"venter_paa_tid":0,"venter_paa_vindue":0,"mails_sendt":0,"dry_run":true,"nu":"2026-09-15T20:23:22.055Z","ville_sende":[],"ville_stemple":[]}` — BEVIST at den nye kode kører (kun den svarer `dry_run` og `udloebet`). **Reglen er UBEVIST i drift:** køen var tom. Bevis = første række til et udløbet medlem: `udloebet ≥ 1` i køens svar og ingen mail i `email_send_log`. Tilføjet til mangellistens «Beviser der udestår». Chatten oprettede bevidst ingen testrække til et rigtigt medlem — en cron-kørsel imellem ville sende for alvor, hvis reglen var forkert.

**16. SAMLEMAILENS INTEGRATION — PR 2 (15/9 sent; bygget og testet — #897 merget 20:40:59 UTC, UDRULLET 20:43 UTC, tørkørsel 200 kl. 20:44 dansk; fordeling, mail og stempling UBEVIST i drift).**

- **CHATTENS beslutninger 15/9 (ikke Jonas'):** **1) Egen hentning:** køens eksisterende hentning (den FØRSTE `.from("notifications") … .limit(50)`) udelukker `SAMLEMAIL_TYPER` — ét filter mere i kæden (`.not("type", "in", "(event_published,community_opslag)")`), intet fjernet eller omdøbt af det dagskvote-værnet låser. Grund: køen tager de 50 ældste ventende rækker hver kørsel; samlemailens rækker venter til kl. 17 og ville fylde alle 50 pladser og stoppe chatsvar og andre mails hele dagen. **2)** Samlemailens rækker hentes i en EGEN forespørgsel EFTER den eksisterende og KUN når `erISamlemailVindue(nu)` er sand: type IN `SAMLEMAIL_TYPER`, `email_sent_at IS NULL`, `seen_at IS NULL`, priority IN (action_required, important), `created_at < nu − 15 min`, ORDER BY created_at, loft 500 (kommentaren i koden: 40 medlemmer × ca. 7 rækker ≈ 280 på en dag; en webinardag med 15 præsentationer × 40 modtagere ≈ 600 tages i to kørsler — cronen kører hvert 5. minut i vinduet, ældste først; en modtager der allerede har fået dagens mail, får resten i morgen, under 48 t er de ikke forældede). Uden for vinduet: ingen hentning, intet sker. **3)** Udløbet-reglen gælder også disse rækker (`fordelUdloebne`; et medlem kan udløbe mellem opslag og kl. 17) — udløbne stemples med `UDLOEBET_GRUND`. **4)** Resten går til `fordelSamlemail({ nu, raekker, opslaaet, modtagere })` — opslag: `events (id, title, starts_at, meet_url, status)` på `reference_id`; `community_traade (id, titel, status, kilde_type, forfatter_id)` på `reference_id`; forfatternes `profiles.full_name`; `community_visninger` for (bruger, tråd) med samme dom som køen (`${user}|${traad}`). Modtagere: rådgiver (samme advisor-opslag som køen), `importantFra` (prefs), email og fornavn (auth + `profiles.full_name` via `fornavnFraFuldtNavn`), `sidstSendt` (seneste `email_send_log` med `template_name = SAMLEMAIL_LABEL` og status `sent` pr. modtagerens mail), `dagskvoteNaaet` (køens `countMap` mod `MAX_EMAILS_PER_DAY` — ikke hentet igen). **5)** Pr. mail: `bygSamlemail({ fornavn, punkter, nu, appUrl: APP_URL })` → `send()` med label `SAMLEMAIL_LABEL`, fordelingens idempotency-nøgle, rækkerne som `raekker`. Ved `!sent`: som køens andre løkker (`skalKoeStoppe` → `rateLimit` og `break`; ellers log og videre uden stempel). Ved `sent`: `stempl(n, "sendt")` for hver række, `countMap[userId] += 1`, `mailsSendt += 1`, `sent += antal rækker`. `stemplesUdenMail` → `stempl(id, grund)`; `venter` → intet. Løkken kører EFTER de to eksisterende, og ikke hvis `rateLimit` allerede er sat. **6)** `nu` fra tørkørslen styrer samlemailens vindue, sidstSendt-dommen, forældelsen og tidsmærkerne (og hentningens 15-minutters grænse). Svaret: `samlemail: { i_vinduet, hentet, udloebet, mails, stemplet_uden_mail, venter }`; i tørkørsel står hver samlemail i `ville_sende` med `type: "samlemail"`, `user_id` og `ids` (row-id'erne).
- **Bygget i `send-notification-email/index.ts`** (og kun der + tre testfiler; `_shared/samlemail.ts` er urørt): import af `SAMLEMAIL_LABEL`, `SAMLEMAIL_TYPER`, `bygSamlemail`, `erISamlemailVindue`, `fordelSamlemail`, `fornavnFraFuldtNavn` og typerne fra `../_shared/samlemail.ts`; filteret i første hentning; samlemailens hentning før den tidlige retur — som nu kræver at BÅDE køen og samlemailens rækker er tomme (ellers ville en tom kø kl. 17 stoppe samlemailen); `userIds` inkluderer samlemailens modtagere, så advisor-, pref-, mail- og kvoteopslagene dækker dem uden nye kald; udløbet-reglen kaldes en gang til på samlemailens rækker (samme `medlemskaber`); løkken efter non-chat-løkken. `processed` tæller nu køens rækker + samlemailens hentede, så `sent + skipped + venter` går op (samlemailens ventende i `samlemail.venter`). Nye variabler, alle erklæret før første brug: `samlemailIVinduet` (261/263), `samlemailRaekker` (262/276), `samlemailUdloebet` (422/427), `samlemailTilFordeling` (430/926), `samlemailSendt` (923/1058), `samlemailStemplet` (924/1020), `samlemailVenter` (925/1023). Værnenes øvrige tællinger er uændrede, fordi rådgiver-, mail- og kvotedommene ligger i `fordelSamlemail`: `advisorUserIds` bruges via aliaset `raadgiverIds` (`raadgiverIds.has(uid)`), mailen som `userEmailMap.get(uid) ?? null`, kvoten som `(countMap[uid] || 0) >= MAX_EMAILS_PER_DAY`.
- **Værnændringerne, ordret:** `dagskvote.test.ts:120` var `expect(kode.split("if (skalKoeStoppe(resultat)) {").length - 1).toBe(2);` → nu `.toBe(3)` med kommentaren «PR 2 (15/9): samlemailens løkke er en tredje afsender-løkke med sit eget rate limit-stop — 2 → 3, bevidst. De øvrige tællinger er uændrede: samlemailens rådgiver-, pref-, mail- og kvotedomme ligger i fordelSamlemail (_shared/samlemail.ts), ikke i index.ts.» `mailFejl.guard.test.ts:50-53` var `.toBe(2)`, `nonChat`-udsnittet til `const summary = {` og `for (const loekke of [chat, nonChat])` → nu `.toBe(3)`, `nonChat`-udsnittet slutter ved `let samlemailSendt = 0;`, nyt udsnit `samlemail` fra `for (const m of fordeling.mails)` til `const summary = {`, og `for (const loekke of [chat, nonChat, samlemail])` — samme «`break;` inden for 900 tegn»-krav på alle tre; kommentaren «PR 2 (15/9): samlemailens løkke (for (const m of fordeling.mails)) er en tredje afsender-løkke med sit eget stop — 2 → 3, bevidst; den får sit eget udsnit med samme «break; inden for 900 tegn»-krav.» `sendNotificationEmail.dryrun.guard.test.ts` holder uændret: stadig præcis ét `sendManagedEmail(` og ét `email_sent_at`-update, begge i hjælperne. Nyt kildeværn `sendNotificationEmail.samlemail.guard.test.ts` (8): første hentning udelukker `SAMLEMAIL_TYPER`; samlemail-hentningen efter den og gated af `erISamlemailVindue(nu)`; den tidlige retur kræver begge tomme; `fordelUdloebne` på samlemailens rækker; `fordelSamlemail` og `bygSamlemail` importeret og brugt; afsendelsen gennem `send(` med `SAMLEMAIL_LABEL` og fordelingens nøgle; stemplingen gennem `stempl(`; løkken efter de to og gated af `!rateLimit`; tællingerne af de tre gamle gates stadig 2; svaret bærer `samlemail`-feltet.
- **Målt:** `bunx tsc --noEmit -p tsconfig.app.json` exit 0; `bun run test`: Test Files 233 passed (233), Tests 3529 passed (3529); `deno check supabase/functions/_shared/samlemail.ts supabase/functions/_shared/mailModtager.ts` grøn (begge); `bun run check:edge-auth`: «PASS — all 68 triggered files contain at least one auth predicate.» index.ts kan fortsat ikke checkes lokalt (npm-importen i managedEmail, §15).
- **Næste skridt:** (1) udrulning — `send-notification-email` OG `_shared/samlemail.ts` eksplicit i build-chatten, `git pull`-output læst bagefter; (2) tørkørsel med `nu` sat til en dansk dag kl. 17:30 (UTC 15:30 i sommertid) når der ligger rækker — svaret skal vise `samlemail.i_vinduet: true`, `hentet ≥ 1` og en `ville_sende`-post med `type: "samlemail"`; (3) beviset i drift = første rigtige samlemail sendt og stemplet (label `notification-samlemail` i `email_send_log`, rækkerne med `email_sent_at`), senest 21/9 — ellers Jonas' fallback: præsentationer som info i to uger. **Efter udrulningen sendes nye events og opslag IKKE længere straks, men samles til kl. 17** — det er den synlige ændring for medlemmerne.
- **UDRULLET 15/9 kl. 20:43 UTC** (#897 = `bab6848a`, merget 20:40:59 UTC), Lovable ordret: «Udrulningen af `send-notification-email` lykkedes kl. 2026-09-15 20:43 UTC fra main (commit bab6848a, PR #897), med de delte filer `_shared/samlemail.ts` og `_shared/mailModtager.ts` med i udrulningen. Bygget gav ingen type- eller byggefejl. Funktionen svarer live med HTTP 403 «Forbidden» ved kald uden service-role-token — forventet adgangsdom. Ingen filer, andre funktioner eller migrationer blev rørt.» `git fetch` bagefter: ingen commits. **TØRKØRSEL 20:44 dansk** (request_id 10835, body `{"dry_run": true, "nu": "2026-09-16T15:30:00Z"}`), svar ordret: status 200, `{"processed":0,"sent":0,"skipped":0,"udloebet":0,"venter_paa_tid":0,"venter_paa_vindue":0,"mails_sendt":0,"samlemail":{"i_vinduet":true,"hentet":0,"udloebet":0,"mails":0,"stemplet_uden_mail":0,"venter":0},"dry_run":true,"nu":"2026-09-16T15:30:00.000Z","ville_sende":[],"ville_stemple":[]}`. Kontroltælling i SQL 22:45 dansk (`query-results-export-2026-09-15_22-45-43.csv`) med samlemailens filtre og `created_at < 2026-09-16T15:15Z`: i alt 0. **BEVIST:** koden kører, køens nye filter virker (ellers 500), vinduet genkendes af `nu`, hentningen svarer til databasen. **UBEVIST:** fordeling, mail og stempling — bevis = første rigtige samlemail (label `notification-samlemail`, status `sent`, rækkerne stemplet), tidligst 16/9 kl. 17 dansk, senest 21/9, ellers fallback. Efter udrulningen sendes events og opslag ikke længere straks. Tilføjet «Beviser der udestår».

**17. VINDUESMAILENE — PR 3 (15/9 sent; TEKSTERNE GODKENDT AF JONAS 15/9; #898 merget 21:17:00 UTC; migrationen KØRT 23:11 dansk FØR commit; UDRULNINGEN VENTER på Lovables vedligehold — hård frist 22/9 kl. 11 UTC).**

- **JONAS' BESLUTNING 15/9 (valg A):** to mails i forlængelsesvinduet — dag 1 («udløbet, du kan stadig forlænge til og med {dato}») og dag 11 («sidste chance, tilbuddet lukker {dato}») — KUN ved beslutning «tilbyd». Vinduet er dag 0–14 efter slutdato.
- **CHATTENS beslutninger 15/9 (ikke Jonas'):** **C1)** vindue 1 er forfalden fra dag 1 til og med dag 10 efter slutdato, hvis ikke sendt (indhentning ved en mistet cron-dag); vindue 2 fra dag 11 til og med dag 14. Er begge forfaldne (dag ≥ 11 uden vindue 1), sendes KUN vindue 2. CARMA STUDIO (slut 2026-09-11, i dag dag 4): vindue 1 ved første kørsel efter udrulningen, vindue 2 den 22/9. **C2)** kun når `afgoerFornyelsestilstand` giver `udloebet_tilbyd` regnet med companies' RIGTIGE abonnementsfelter — et medlem med aktivt abonnement er selvbetjener, ikke udløbet; varslerne FØR slutdato regnes som i dag. **C3)** stempler `vindue_1_sendt_at` og `vindue_2_sendt_at` på `company_fornyelse`, sat KUN ved succes; migration `20260915230000_fornyelse_vinduesstempler.sql` i samme form som `20260907090000` (navngivne kolonner, kommentar pr. kolonne, KØRES MANUELT). **C4)** pris, afsender og modtager som varslerne: `beregnFornyelsespris` (intet vinduesmail uden pris), `sendIndgangsMail`, `contact_email`, tiltale af `fornavnAf(contact_person)`; labels `fornyelse-vindue1` og `fornyelse-vindue2`; knap som varslerne til forsiden (tilbudsskærmen); «til og med {dato}» = slutdato + `FORNYELSE_TILBUDSVINDUE_EFTER_UDLOEB_DAGE` (14), formateret med `formatDanskDato`. **C5)** teksterne nedenfor, i husets tone, ingen løfter koden ikke holder.
- **TEKSTERNE, ORDRET — GODKENDT AF JONAS 15/9.** Jonas, ordret: «Vindue 1 er fint. Vindue 2 skal ikke have "Så skal du skrive til os" med. Det må de selv finde ud af, for ellers åbner vi et smuthul, som egentlig ikke er meningen.» Og: «vores mails skal give dem en hård deadline.» Vindue 2's afsnit 1 er rettet derefter (sætningen «Derefter kan det ikke fornys med et par klik — så skal du skrive til os.» er fjernet; testen kræver nu at mailen hverken indeholder «skriv til os», «skrive til os», «par klik» eller kontaktadressen). Teksterne (`_shared/fornyelsesMail.ts`, `vindue1Mail`/`vindue2Mail`; {fornavn}, {virksomhed}, {slutdato}, {til og med}, {pris} udfyldes; eksemplet er CARMA: Camilla, CARMA STUDIO, 11. september 2026, 25. september 2026, 20.000):
  - **Vindue 1 — emne:** «Dit medlemskab er udløbet — du kan stadig forlænge til og med {til og med}»
    **Overskrift:** «Kære {fornavn},» (uden fornavn: «Kære,»)
    **Afsnit 1:** «Dit medlemskab af The Boardroom for {virksomhed} udløb {slutdato}, og adgangen til platformen er lukket. Dine tal, dine rapporter og din plads i netværket ligger stadig klar.»
    **Afsnit 2:** «Du kan forlænge til og med {til og med}. Prisen for det næste år er {pris} kr. ekskl. moms, og adgangen åbner igen, så snart betalingen er gået igennem.»
    **Knap:** «Forny medlemskabet» → https://app.theboardroom.dk/ (forsiden; gaten viser tilbuddet)
    **Efter knappen:** «Har du spørgsmål, så skriv til kontakt@theboardroom.dk.»
    **Hilsen:** «Venlig hilsen / Jonas Herlev»
  - **Vindue 2 — emne:** «Sidste chance: tilbuddet om at forlænge lukker {til og med}»
    **Overskrift:** «Hej {fornavn},» (uden fornavn: «Hej,»)
    **Afsnit 1:** «En sidste påmindelse: tilbuddet om at forlænge medlemskabet for {virksomhed} lukker {til og med}.»
    **Afsnit 2:** «Prisen er {pris} kr. ekskl. moms for det næste år.»
    **Knap:** «Forny medlemskabet» → forsiden. **Hilsen:** «Venlig hilsen / Jonas Herlev»
  - **Hvorfor hvert løfte holder:** «adgangen … er lukket» (tier expired → `MembershipExpiredGate`); «ligger stadig klar» (data slettes tidligst 45 dage efter slutdato, `sletning.ts`); «til og med {til og med}» (`udloebet_tilbyd` til dag 14); «adgangen åbner igen, så snart betalingen er gået igennem» (`stripe-webhook`s fornyelsesgren sætter `active` og ny slutdato). Vindue 2 siger ikke hvad der sker efter datoen — en hård deadline uden smuthul (Jonas 15/9).
- **Motoren (`src/lib/fornyelsesvarsel.ts` + `_shared`-spejl, pariteten holder):** udfaldet udvidet på SAMME felt — `Varselsnummer = 1 | 2 | "vindue_1" | "vindue_2"` (husets form: højst ét forfaldent, det højeste trin vinder; ikke en ny art ved siden af). Konstanter `VINDUE_1_FRA_DAG` = 1, `VINDUE_1_TIL_DAG` = 10, `VINDUE_2_FRA_DAG` = 11, `VINDUE_2_TIL_DAG` = 14 (fra fornyelse.ts). Input får `vindue_1_sendt_at`, `vindue_2_sendt_at`, `subscription_status`, `subscription_current_period_end` — alle KRÆVEDE felter, så kalderen siger dem eksplicit (eneste kalder: cronen). Abonnementsfelterne sendes nu til `afgoerFornyelsestilstand` (før: null). Gren 4 («slutdato passeret»): er tilstanden `udloebet_tilbyd` → vinduesdommen; alt andet → intet som før, med grunden («tilbudsvinduet er lukket», «medlemmet er selvbetjener (aktivt abonnement)», stemplet). Dag 0 er stadig varslernes. Tests: dag 0 intet (varsler sendt); dag 1 og 10 vindue 1; dag 10 stemplet intet; dag 11 vindue 2 (også uden vindue 1 — kun vindue 2); dag 14 vindue 2; dag 15 intet; tilbyd_ikke intet; aktivt abonnement intet (udløbet abonnement → vindue 1); abonnementsfelterne ændrer intet før slutdato; CARMA-datoerne konkret (12/9, 15/9, 21/9 vindue 1; 22/9, 25/9 vindue 2; 26/9 intet).
- **Testændringerne, ordret (bevidste, med kommentar i filen):** `fornyelsesvarsel.test.ts:163-171` var «dagen efter slutdato: intet — et varsel om noget der er sket, er forkert; tilbuddet er tilstandsmotorens sag» (`input(-1)` → `{ varsel: null, grund: "intet: slutdatoen er passeret for 1 dag siden", dage_til_udloeb: -1 }`; `input(-10).grund` → «… 10 dage siden») → nu «dagen efter slutdato med tilbyd: VINDUE 1 (15/9) — dag 1 og dag 10 er begge vindue 1» (`input(-1)` → `{ varsel: "vindue_1", grund: "vindue 1 forfaldent: slutdatoen er passeret for 1 dag siden; kan forlænge til og med dag 14", dage_til_udloeb: -1 }`). `:197-216` (sweepet «uden for ordningen») var «Efter slutdatoen vinder gren 4 (passeret) — den er også «intet»» med `expect(ud.varsel).toBeNull()` for alle → nu: FØR slutdato uændret (`blokeret_af uden_for_ordningen`); EFTER slutdato dag ≤ 10 `vindue_1`, dag 11–14 `vindue_2`, dag 15+ null — fordi `afgoerFornyelsestilstand` dømmer udløbet FØR ordningens grænse (tilbuddet står i gaten, bevist for Studio Mini). `:231-236` var «gren 1-4 vinder stadig før gren 5: … passeret slutdato bærer IKKE blokeret_af» → nu «… passeret med tilbyd er vindue 1» (samme `blokeret_af`-assert, plus `varsel === "vindue_1"`). Alle literale inputs og `input()`-basen har fået de fire nye felter (`TOMME_VINDUER`). `fornyelsesvarselParitet.test.ts`: sweepet −20…+60 dækker nu også fire vinduesstempel-kombinationer og to abonnementer (null / aktivt til 2027), og kræver at alle fem udfald rammes; fem nye eksplicitte paritetstilfælde efter slutdato. `varselStempel.guard.test.ts`: nyt describe — cronens `company_fornyelse`-select bærer begge vinduesstempler, companies-select bærer abonnementsfelterne, stempelfeltet vælges pr. udfald i samme update, migrationen definerer begge kolonner. Fladerne (AdvisorDashboard, useVirksomhed) læser IKKE de nye stempler i PR 3 — forsidens dom og badget er uændrede; værnets to første describes er urørte.
- **Byggerne** (`_shared/fornyelsesMail.ts`): `vindue1Mail`/`vindue2Mail` med `VinduesMailArgs` (varslernes args + `tilOgMedDato`), labels `LABEL_VINDUE_1`/`_2`; tests i `fornyelsesMail.test.ts` (emne, tiltale med/uden fornavn, datoerne, prisen, knappens URL, escaping, ingen Stripe, kortere end vindue 1, fire forskellige labels).
- **Cronen** (`fornyelsesvarsel-cron/index.ts`): henter `vindue_1_sendt_at, vindue_2_sendt_at` og `subscription_status, subscription_current_period_end`; motoren får alle felter; tæller `vindue_1`/`vindue_2` i svaret og viser dem i `forfaldne[]` (`varsel` er nu også `"vindue_1"`/`"vindue_2"`); mail, label og stempelfelt pr. udfald; `tilOgMedDato()` = slutdatoens UTC-dag + 14. Nye variabler, erklæret før første brug: `tilOgMedDato` (funktion, linje 176 / brug 354), `vindueArgs` (354 / 358). Tørkørsel er stadig standard (tom body).
- **Migrationen** `20260915230000_fornyelse_vinduesstempler.sql`: `add column if not exists vindue_1_sendt_at timestamptz, vindue_2_sendt_at timestamptz` med kommentar pr. kolonne, husets filhoved (KØRES MANUELT, hvorfor, to navngivne kolonner, fire konsekvenser, bevis-SELECT). KØRT 15/9 kl. 23:11 dansk i SQL editor FØR commit, i én transaktion med måling (`query-results-export-2026-09-15_23-11-00.csv`): før 0 kolonner; efter `vindue_1_sendt_at` og `vindue_2_sendt_at`, begge `timestamp with time zone`, kommentarerne ordret som i filen. Var: «IKKE KØRT».
- **Målt:** `bunx tsc --noEmit -p tsconfig.app.json` exit 0; `bun run test`: Test Files 233 passed (233), Tests 3552 passed (3552); `deno check supabase/functions/_shared/fornyelsesvarsel.ts supabase/functions/_shared/fornyelsesMail.ts` grøn (begge); `bun run check:edge-auth`: «PASS — all 68 triggered files contain at least one auth predicate.» `deno check supabase/functions/fornyelsesvarsel-cron/index.ts` kan ikke køres lokalt, ordret: «error: Could not find a matching package for 'npm:@lovable.dev/email-js@0.1.0' in the node_modules directory. Ensure you have all your JSR and npm dependencies listed in your deno.json or package.json, then run `deno install`. Alternatively, turn on auto-install by specifying "nodeModulesDir": "auto" in your deno.json file. at file:///…/supabase/functions/_shared/managedEmail.ts:15:49» (via `sendIndgangsMail` → `managedEmail`; DEL 4).
- **RÆKKEFØLGEN FOR UDRULNING:** (1) teksterne godkendt (Jonas 15/9, ovenfor); (2) migrationen `20260915230000` køres i SQL editor FØR commit — den tilføjer kun kolonner, som den nuværende cron ignorerer, mens PR'en ændrer cronens egen index.ts, og kører den nye kode før migrationen, fejler hele cronen, også varslerne; bevis `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'company_fornyelse' AND column_name IN ('vindue_1_sendt_at','vindue_2_sendt_at')` før (0 rækker) og efter (2); (3) commit og merge; (4) `fornyelsesvarsel-cron` og de ændrede `_shared`-filer (`fornyelsesvarsel.ts`, `fornyelsesMail.ts`) udrulles eksplicit i build-chatten, `git pull` læst bagefter; (5) tørkørsel (cronens tomme body ER tørkørsel) — CARMA STUDIO skal stå i `forfaldne[]` med `varsel: "vindue_1"`, `vindue_1: 1`, modtager camilla@carmastudio.dk og pris — og `cron.job`-jobbet «fornyelsesvarsler» målt (`SELECT jobid, jobname, schedule, active FROM cron.job WHERE jobname = 'fornyelsesvarsler'`); (6) den næste planlagte kørsel sender vindue 1 til CARMA og stempler; vindue 2 følger 22/9. Bevis = `email_send_log` label `fornyelse-vindue1`, status `sent`, og `vindue_1_sendt_at` sat.
- **STATUS 15/9 kl. 23:25 dansk:** (1) teksterne godkendt; (2) migrationen KØRT 23:11 (ovenfor); (3) #898 = `c80c8d1b`, merget 21:17:00 UTC, CI grøn på `8b269648` («Tests» og «Edge Function Auth Guardrail»); (4) **UDRULNINGEN VENTER:** Lovable i vedligehold 15/9 ca. 23:20 dansk — build-chatten ordret: «Waiting for the backend maintenance window to end before retrying»; forsiden: «We’re currently undergoing maintenance»; forventet tilbage 23:45 — og SQL editoren kunne heller ikke køre tørkørslen. Den gamle cron kører videre og ignorerer de nye kolonner. Om merge selv udrullede cronen: ikke målt. **HÅRD FRIST:** `fornyelsesvarsel-cron` + de to delte filer (`fornyelsesvarsel.ts`, `fornyelsesMail.ts`) udrullet før cronens kørsel kl. 11 UTC 22/9 (CARMA's dag 11); vindue 1 til CARMA kan gå til og med 21/9 (dag 10). (5) og (6) står.

**18. AFTENENS REST (15/9 kl. 22:45–23:25) — DE TYVE (9), (13), (16) OG CVR-OPSLAGET.**

- **DE TYVE (9) SKREVET OM — MÅLT 15/9 kl. 22:55 dansk** (`query-results-export-2026-09-15_22-55-21.csv`) på dagens nye medlem **Din Forsikringsret** (oprettet 15:34:19 UTC ad Monday-vejen; owner logget ind 15:42 UTC; velkomsten set 15:45 UTC; kontrakt til 2027-09-15; CVR 45728064; website `https://dinforsikringsret.dk/`; `industry_label` TOM; `ask_me_about` tom; deling ikke hentet). Kortets præmis «website sættes ikke» er FORKERT: `monday-webhook` sender website fra kolonnen «Hjemmeside» (`:274`, uændret siden 2/9, `recon-tjeklisten-monday-vejen.md` §3–4) og gjorde det i drift. Det der mangler er BRANCHEN: Monday-kolonnen «Branche» var tom, og CVR-opslaget fejlede tavst (skærm 21:37: «CVR-opslag mangler»), så tjeklistens «Din virksomhed» står med «Mangler: branche» — tjeklisten viser «1 af 8» efter velkomsten. Et nyt fuldt medlem ser 8 punkter efter #886. Kortet (n14-12) og DEL 3-rækken er skrevet om til «Branchen mangler ad Monday-vejen når CVR-opslaget fejler», med henvisning til (16); den gamle præmis mærket RETTET, intet slettet.
- **CVR-OPSLAGET** (`recon-cvr-opslaget.md`, vindue B): `hentCvrData` kalder cvrapi.dk anonymt (ingen nøgle, ingen timeout, `_shared/virksomhedsOprettelse.ts:64-102`); ved enhver fejl returneres null, og oprettelsen fortsætter tavst uden `industry_code`, `cvr_fetched_at` og `raw_cvr_data` (`:74-101`, `:135-137`); dommen «CVR-opslag mangler» (`cvrBerigelse.ts:64-72`) læser `address` og `industry_code`, tjeklisten læser `industry_label` — to forskellige felter. **BERIGELSEN KØRT 15/9:** tørkørsel 23:04 (request_id 10842): status 200, 32 aktive, `ville_bruges` 2 (Din Forsikringsret: `industry_code` + `industry_label`; Nordic By Hand ApS: `industry_code`), `contact_email` 0, uden CVR: Alexander Lunds virksomhed, Bastant Design, Martin Larsens virksomhed. Rigtig kørsel 23:05 (request_id 10844, body `{"dry_run": false}`): status 200, opslag brugt 2, `uden_svar` 2, `virksomheder_aendret` 0, `cvr_uden_svar` [Din Forsikringsret, Nordic By Hand ApS] — cvrapi svarede ikke fra edge-runtimen; INTET skrevet (felterne tomme før og efter). **Konsekvens for 22/9:** berigelsen kan ikke hjælpe så længe cvrapi afviser Supabase; nye ad Monday-vejen vil sandsynligvis mangle branchen fra start; branchen sættes af medlemmet på /settings eller af rådgiveren i «Rediger virksomhedsdata».
- **VIRK — DE TYVE (16):** Jonas sendte 15/9 ca. 23:13 dansk en ansøgning om «system-til-system adgang til CVR-data» til cvrselvbetjening@erst.dk (formål: stamdata på medlemsvirksomheder ud fra CVR-nummer; få opslag om dagen; tilbud om at underskrive erklæringen om reklamebeskyttede enheder; spørgsmål om sagsbehandlingstid). Slået op 15/9 (datahub.virk.dk og erhvervsstyrelsen.dk): adgangen gives efter mail til cvrselvbetjening@erst.dk, med erklæring og sagsbehandlingstid; derefter brugerId og password til Elasticsearch på distribution.virk.dk. Svar afventes; usikkert om før 22/9. Kortet og DEL 3-rækken: «Ansøgning sendt 15/9 — svar afventes».
- **DE TYVE (13) MONDAY-OPRYDNINGEN:** betingelsen «kæden bevist på et rigtigt Godkendt» er opfyldt 15/9 — Jonas ordret: «Godkendt i mandag og den blev grebet af platformen 100% og der er betalt»; målt i SQL 22:55 (Din Forsikringsret ovenfor). De fire døde webhooks (185102661, 161201327, 161201319, 157778880) kan ryddes; IKKE gjort. DEL 3-rækken og kortet: «Betingelsen opfyldt 15/9 — klar til oprydning».

**19. FUND K — APP-LINKETS PREVIEW-TEKST (DE TYVE (11)); RETTET I KODE 15/9, UBEVIST (kræver Update-klik).**

- **JONAS 15/9, valg A, ordret:** titel «The Boardroom — sparring for virksomhedsejere»; beskrivelse «12 måneder med sparring fra Morten Larsen og Jonas Herlev og et netværk af andre selvstændige. Medlemskab efter ansøgning på theboardroom.dk.».
- **Rettet i kode:** `index.html` — `<title>`, `meta description`, `og:title`, `og:description`, `twitter:title` og `twitter:description` bærer teksten ordret (titlen i de tre titel-tags, beskrivelsen i de tre beskrivelses-tags); UÆNDRET: `robots noindex, nofollow`, canonical, `og:url`, `og:type`, `og:image` med width/height/alt, `twitter:card`, `twitter:image`. `public/manifest.json` — `description` er beskrivelsen ordret; `name` og `short_name` uændrede. Nyt kildeværn `src/lib/__tests__/linkPreview.guard.test.ts`: de seks tags, den gamle tekst væk («Alt-i-én platform», «Dit finansielle overblik», «Rapportér, budgettér»), robots noindex stadig, og:image stadig `/og-image.jpg` 1200×630, manifestets description = beskrivelsen og uden «rapportér». Grep i `src/`, `public/` og `index.html` for «Dit finansielle overblik» og «Alt-i-én platform» FØR rettelsen: kun `index.html:7, :8, :19, :20, :27, :28` og `public/manifest.json:4` — ingen andre steder; efter rettelsen kun værnets egne negative assertions. Målt: `bunx tsc --noEmit -p tsconfig.app.json` exit 0; `bun run test`: Test Files 234 passed (234), Tests 3557 passed (3557).
- **Fra `recon-fund-k-link-preview.md` (vindue B):** delingen udleverer ALDRIG app-linket — de fire tekster (`delingstekster.ts`) har intet link, VEJLEDNING sender theboardroom.dk til første kommentar, og der findes ingen «del link»/«kopiér link» i appen (ingen `navigator.share`). Men app'en er en ren SPA (`vite build`, ingen SSR/prerender, ingen dynamisk meta), så ethvert app-link — også `/deling` fra adresselinjen — viser `index.html`'s tags. Frontend — kræver Update-klik i Lovable.
- **BEVIS** = LinkedIns Post Inspector på `https://app.theboardroom.dk` viser den nye titel og beskrivelse (LinkedIn cacher; Post Inspector henter på ny). Ubevist til da. Der fandtes ingen egen DEL 3-række for fund K (kun DE TYVE (11)) — der er tilføjet én.
...
|---|---|---|
| **MERGET 15/9 (#898 `c80c8d1b`, 21:17:00 UTC; CI grøn); teksterne GODKENDT af Jonas; migrationen `20260915230000` KØRT 23:11 dansk FØR commit (målt: begge kolonner); UDRULNINGEN VENTER — Lovable i vedligehold ca. 23:20 dansk; HÅRD FRIST: cron + de to delte filer udrullet før kørslen kl. 11 UTC 22/9 (CARMA's dag 11; vindue 1 kan gå t.o.m. 21/9). Var: BYGGET 15/9 sent — PR 3; IKKE udrullet** | **Motoren svarer nu `"vindue_1"` (dag 1–10) og `"vindue_2"` (dag 11–14) for `udloebet_tilbyd`, regnet med de rigtige abonnementsfelter; byggerne `vindue1Mail`/`vindue2Mail`; cronen sender ad samme vej og stempler `vindue_1_sendt_at`/`vindue_2_sendt_at` (migration `20260915230000`, ikke kørt). Testændringerne står ordret i §17. Rækkefølge: godkend tekster → merge → migration i SQL editor → cron + `_shared` udrullet eksplicit → tørkørsel (CARMA med vindue_1) → kørslen kl. 11 UTC sender. CARMA STUDIO: vindue 1 ved første kørsel efter udrulning, vindue 2 den 22/9. | DEL 2 «15. september» §17; `recon-koe-regel-og-vinduesmails.md` §5 (uden for repoet); mangellisten (m15-vinduet) |
| **UDRULLET 15/9 kl. 20:43 UTC (#897 `bab6848a`, merget 20:40:59 UTC; `samlemail.ts` og `mailModtager.ts` med); tørkørsel 20:44 dansk med `nu` 16/9 kl. 17:30 → 200, `samlemail.i_vinduet: true`, hentet 0 (kontroltalt 0 i SQL) — koden kører, filteret og vinduet virker; fordeling, mail og stempling UBEVIST: bevis = første rigtige samlemail 16/9 kl. 17 dansk, senest 21/9. Var: BYGGET 15/9 sent — PR 2; IKKE udrullet** | **Samlemailens typer er ude af køens første hentning og hentes i egen forespørgsel kun i vinduet 17–20 dansk (loft 500); udløbet-reglen på dem; `fordelSamlemail` → `bygSamlemail` → `send()` med label `notification-samlemail` og idempotency-nøgle pr. dansk dato; løkken efter de to eksisterende. Chattens beslutninger 1–6. Værnene ændret bevidst (2 → 3 `skalKoeStoppe`, ordret i §16); nyt kildeværn. Næste: udrulning (functionen OG `_shared/samlemail.ts`), tørkørsel med `nu` kl. 17:30 dansk når der ligger rækker, bevis = første rigtige samlemail senest 21/9 — ellers fallback (præsentationer som info i to uger). Efter udrulningen samles events og opslag til kl. 17. | DEL 2 «15. september» §16, §12; mangellisten (n14-11, w-h) |
| **UDRULLET 15/9 kl. 20:21 UTC (#896 `8216ad61`, merget 20:19:14 UTC; `mailModtager.ts` med); tørkørsel fra SQL editoren 20:23 → 200 med `dry_run: true` og `udloebet: 0` — den nye kode kører; REGLEN UBEVIST i drift (køen var tom). Var: BYGGET 15/9 sent — PR 1; IKKE udrullet** | **`send-notification-email` stempler nu rækker til brugere hvis virksomheder ALLE er expired (grund «udloebet», `_shared/mailModtager.ts`), før chat-grupperingen og udvælgelsen — chattens beslutning på Jonas' krav «ingen platformsmails i forlængelsesvinduet». Tørkørsel med body `{"dry_run": true}` (+ valgfrit `nu`): intet sendes, intet stemples, svaret siger hvad der ville ske. Værnene dagskvote/mailFejl urørte og grønne; nyt kildeværn `sendNotificationEmail.dryrun.guard.test.ts`. Udrulning: functionen OG den nye delte fil eksplicit i build-chatten; derefter tørkørsel fra SQL editoren (`net.http_post`, body `{"dry_run": true}`). | DEL 2 «15. september» §15; `recon-koe-regel-og-vinduesmails.md` (uden for repoet) |
| **ÅBENT 15/9 — arbejdsgang for Jonas og Morten, ikke kode (DE TYVE (6), instruks J)** | **J — e-mailkolonnen FØR «Godkendt» i Monday:** målt i kode 15/9: `monday-webhook/index.ts:264` svarer `{ok, skipped, reason: "ingen_email"}` når mailkolonnen er tom ved «Godkendt» — intet betalingslink, ingen besked til nogen; virksomheden oprettes ikke. Sæt mailen på Monday og sæt «Godkendt» igen. | DEL 2 «15. september» §15; DEL 2 «14. september» §46 (6) |
| **ÅBENT 15/9 — arbejdsgang for Jonas og Morten, ikke kode (DE TYVE (6), instruks I)** | **I — importvejen er IKKE til webinar-ansøgerne:** målt i kode 15/9: `import-application` opretter virksomheden (`:123-147`) uden `company_betalingslink` og sætter kun kontraktdatoer fra regnearket (`:165-184`; kommentaren: slutdato læses som «har betalt»), så en importeret webinar-ansøger får adgang uden at betale. Ansøgerne skal gennem Monday-vejen. | DEL 2 «15. september» §15; DEL 2 «14. september» §46 (6) |
| **ÅBENT 15/9 — FORLÆNGELSESVINDUET: udløbne medlemmer i vinduet får platformens mails, mangler vinduesmails (Jonas: dag 1 og 11, kun ved tilbyd) og kan ikke findes på /virksomheder; recon i gang (vindue B). RETTET 15/9 (§15): rapportpåmindelser rammer dem IKKE (send-report-reminder:353); /virksomheder FINDER dem ved søgning (bevist på skærm 21:54, Jonas: «Det virker jo faktisk fint.»); ingen mails er gået til CARMA/Studio Mini siden slutdatoen (målt 21:49). Tilbage: køens regel (PR 1 BYGGET, ikke udrullet) og vinduesmailene (PR 3). PR 1 UDRULLET 20:21 UTC; PR 3 BYGGET 15/9 sent, ikke udrullet — teksterne afventer Jonas (§17)** | **Et udløbet medlem i forlængelsesvinduet (`udloebet_tilbyd`, dag 0–14) behandles som aktivt.** `send-report-reminder` filtrerer kun `status = 'active'`; chat/broadcast/KPI skriver til alle `company_members`; event B/C og aflysning rammer tidligere tilmeldte; køen har ingen medlemskabsdom. Jonas 15/9: ikke rapportpåmindelser og alt muligt andet indefra platformen; en til to mails i perioden — valg A: dag 1 og dag 11, kun ved «tilbyd». /virksomheder: oversigten uændret, men de skal kunne findes. Målt 15/9 21:37: «Viser 27 af 29» — CARMA, Studio Mini, Alexander Lund, Martin Larsen og Topix.dk er `active` men ikke på listen; filtrene IKKE målt. Chattens retning: én mailregel i køen og hver strøm; vinduesmails stemplet som varslerne; søgning uden for oversigten; `afgoerFornyelsestilstand` er tilstanden, ingen ny kolonne. | DEL 2 «15. september» §14; `recon-medlem-der-ikke-fornyer.md`, `recon-forlaengelsesvinduet.md` (vindue B, uden for repoet); mangellisten (Betaling og fornyelse) |
| **LUKKET 15/9 kl. 21:55 — SKREVET (`query-results-export-2026-09-15_21-55-49.csv`: én række, FØR «» = ''); målt efter 21:56 (`…21-56-15.csv`): «Camilla Risager», aktive uden kontaktperson 0. På /virksomheder vises «Carma Studio» som kontaktperson — ejerens profilnavn (#621), ikke contact_person. Var: ÅBENT 15/9 — SQL i hånden, ikke kode** | **Camilla Risager som CARMA STUDIO's `contact_person`** (Jonas 15/9). Holdt ude 15/9 aften på den forkerte præmis «tidligere medlem → personfelter tømmes» (§39 «KØRT 15/9», RETTET). CARMA er udløbet i vinduet med åbent tilbud til 25/9. Samme form som trin 2 af de 16: guard på tomt felt, tjek på præcis 1. | DEL 2 «15. september» §14; §39 |
...
| **RETTET I KODE 15/9 — fund K (DE TYVE (11)); frontend, kræver Update-klik; UBEVIST til Post Inspector viser den nye tekst** | **App-linkets preview bærer nu Jonas' valg A:** titel «The Boardroom — sparring for virksomhedsejere», beskrivelse «12 måneder med sparring fra Morten Larsen og Jonas Herlev og et netværk af andre selvstændige. Medlemskab efter ansøgning på theboardroom.dk.» — i `index.html` (title, description, og:, twitter:) og `public/manifest.json`; robots noindex og og-billedet uændrede; kildeværn `linkPreview.guard.test.ts`. Delingen udleverer aldrig app-linket, men app'en er en ren SPA, så ethvert app-link viser tags'ene. Bevis = LinkedIns Post Inspector på app.theboardroom.dk. | DEL 2 «15. september» §19; `recon-fund-k-link-preview.md` (uden for repoet); mangellisten (w-k) |
| **DE TYVE FØR 22/9 — prioriteret 15/9 kl. 00:30 (chattens rækkefølge, kortene uændrede)** | **Frist:** (1) ti gamle Stripe-abonnementer annulleres FØR 20/9 — GJORT 15/9 (elleve, kl. 17:05–17:37 dansk) · (2) BR Rosets ti måneder FØR hendes gamle annulleres — GJORT 15/9 (startdatoen rettet, 7 + 5 = 12) · (3) Launch Lab efter trækket 15/9 — GJORT 15/9 (`sub_sched_1UFyOj…`) · (4) Studio Minis tilbyd-række før 20/9. **De nye:** (5) fanout → én mail · (6) de tre instrukser · (7) SQL for de 35 uden contact_person · (8) «allerede accepteret» · (9) tjeklisten 0 af 7 · (10) gæstens grænse · (11) fund K — RETTET I KODE 15/9, ubevist (Update-klik, Post Inspector) · (12) fakturateksten. **Oprydning:** (13) Monday-webhooks · (14) stripe_customer_id for tretten · (15) bevis delingspunktet · (16) Virk-nøglen. **Indhold:** (17) kursusbeskrivelser · (18) præsentationsudkast · (19) eventlokation · (20) notifikation ved like. **Kun fem: 1, 2, 5, 6, 7.** | DEL 2 «14. september» §46; (1)–(3): DEL 2 «15. september» §1, §3, §4 |
...
| **SKREVET OM 15/9 — Branchen mangler ad Monday-vejen når CVR-opslaget fejler (DE TYVE (9)); hænger på (16). Var: NYT 14/9 — fra gennemkørslen; ingen kode planlagt** | **RETTET 15/9 (§18): præmissen «website sættes ikke» var forkert** — `monday-webhook` sender website fra «Hjemmeside» (`:274`) og gjorde det i drift for Din Forsikringsret (målt 22:55). Det der mangler er branchen: Monday-kolonnen «Branche» tom + CVR-opslaget fejlede tavst → «Mangler: branche», tjeklisten «1 af 8» efter velkomsten. Var: **Tjeklisten er 0 af 7 ad Monday-vejen mod 1 af 7 ad importvejen.** «Din virksomhed» dømmes gjort på website + branche + CVR; importvejen får alle tre fra regnearket, Monday-vejen sætter branche-label og CVR, ikke website — punktet står ugjort. Reconen (§16) antog 1 af 6/7 fra start — det gælder kun importen. Et nyt medlem fra Monday ser altså «0 af 7» ved første login. | DEL 2 «15. september» §18; DEL 2 «14. september» §24; mangellisten (Indgangen, n14-12) |
...
| **LUKKET 15/9 kl. 22:29:44 UTC — DE TYVE (4): Jonas valgte «tilbyd ikke» på virksomhedssiden med noten «Forlænger ikke (Jonas 6/9)»; målt 22:30 dansk: `tilbyd_ikke`, `besluttet_af` jonas@topix.dk, slut 2026-09-05, ingen varsler; FØR «tilbyd» iflg. docs (den oprindelige `besluttet_at` er overskrevet af knappen). CARMA STUDIO uændret: tilbyd, besluttet 3/9, varsel 2 sendt 7/9. Konsekvens: tilbudsskærmen viser Studio Mini «Skriv til os»; ingen vinduesmails kan ramme dem; CARMA er den eneste i vinduet med tilbyd. Var: LØST 7/9 (#678) — vinduet; Studio Minis række er nu en BESLUTNING om timing** | **Tilbudsvinduet efter udløb er en tilstand:** `udloebet_vindue_lukket`, kun efter `tilbyd`, fra dag 15 efter slutdato; bevist i drift kl. 09:36–09:38 (DEL 2 «Fornyelseskæden»). **Studio Mini (slut 5/9, `tilbyd`) FORLÆNGER IKKE (Jonas 6/9):** i dag er de dag 2 i vinduet; fra 20/9 lukker vinduet af sig selv, og rækken bliver `udloebet_vindue_lukket` uden at nogen rører den. Beslutningen er om den skal ryddes FØR — indtil da viser gaten dem et tilbud. CARMA STUDIO (7/9, `tilbyd`) håndteres manuelt i dialog. **15/9:** intet ændret; Jonas' beslutning 6/9 står (`fornyelseskaeden-1-september.md:567-575`), rækken er stadig «tilbyd», og vinduet lukker af sig selv efter 19/9 (dag 14; 20/9 er dag 15 → `udloebet_vindue_lukket`). Ingen vinduesmails til dem uden Jonas' ord (DEL 2 «15. september» §14). | fornyelsesordningen §3; fornyelseskæden §13.4, §14; DEL 2 «15. september» §17; `query-results-export-2026-09-15_22-30-09.csv` |
