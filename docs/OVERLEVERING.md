...
> ## 14/9 — START HER (skrevet 14/9 formiddag; HEAD `66b4a732` = #852, merget 14/9 kl. 08:01:50 UTC; #851 = `72d25525`, merget 07:43:29 UTC. Blokken «13/9 — START HER» nedenfor er historik.)
>
...
> **Otte nye kort på mangellisten** (DEL 2 «14. september» §8) og fire nye fælder i DEL 4.
>
...
| **BESLUTTET 14/9 — Jonas opretter nøglen; ÅBENT: bygningen** | **CVR-kilden skiftes fra cvrapi.dk til Virk/Erhvervsstyrelsens CVR-API.** cvrapi's kvote hænger på IP (10:37 200 fra Jonas' maskine, 10:40 QUOTA_EXCEEDED fra edge-runtimen, samme CVR). Ingen nøgle findes i secret-listen eller repoet. Og uafhængigt af kilden: **en fejlet CVR-berigelse skal være synlig for rådgiveren** — i dag er kvitteringen ordret den samme, og `cvr_fetched_at IS NULL` er eneste spor. | DEL 2 «14. september» §6; mangellisten, to kort (Indgangen) |
| **ÅBENT 14/9 — fornyelsesmailen går til PHILBERT 22/9** | **To mails lover svar fra noreply:** `indgangsMail.ts:163` (dag 0: «skriv til mig») og `fornyelsesMail.ts:200` («svar på denne mail eller skriv til jonas@topix.dk»). Kontaktadressen er kontakt@theboardroom.dk (besluttet 14/9); invitationsmailen bruger den allerede (#852). | DEL 2 «14. september» §8; mangellisten (Betaling og fornyelse) |
