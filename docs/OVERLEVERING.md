...
> ## 14/9 — START HER (skrevet 14/9 formiddag; OPDATERET 14/9 MIDDAG efter #853–#857 — HEAD `588db663` = #857, merget 10:00:35 UTC; #856 = `0f4d8396` 09:56:59; #855 = `b60df81d` 09:48:17; #854 = `0fd6d216` 09:44:47 (formiddagens bogføring); #853 = `c115e737` 09:23:16; #852 = `66b4a732` 08:01:50; #851 = `72d25525` 07:43:29 UTC. Blokken «13/9 — START HER» nedenfor er historik.)
>
...
> **Mangellisten:** otte nye kort formiddag (§8); middag: fire lukket (fund G #853, den tavse berigelse #855, de to mails #856, fund A #849 — med forbehold), tre nye (mailloftet, YKRG's CVR, berigelsens NOT_FOUND), to omskrevet (kort 6 og 7 af 18) — §16, §17. DEL 4: fire nye fælder formiddag, fire middag.
>
...
- **Dommen** ren i `_shared/mailFejl.ts:55` (`klassificerMailFejl`): spærret modtager → `recipient_suppressed` (ikke retryable); 429 → `rate_limited`, retryable, med `retryAfterSeconds` (kan være null); alt andet → `failed`, retryable ved 5xx (`erRetryableStatus`, `:51`). `managedEmail.ts` returnerer en union med tre grene. `email_send_log` får status `rate_limited` — kolonnen har ingen CHECK, `EmailLogView.tsx:66` kender værdien («Rate-limited»), ingen migration.
...
| **ÅBENT 14/9 — rammer hver import fra webinaret** | **Importen skriver aldrig `contact_person`.** Rækkebyggeren (`_shared/virksomhedsraekke.ts:171-211`) lægger navnet i `application_context.contact_name`; kolonnen får sin `DEFAULT ''`. Kendt siden 2/9 (`20260902190000:22-27`), aldrig rettet. Kun `monday-webhook` skriver kolonnen. Nordic By Hand sat i hånden 10:14. | DEL 2 «14. september» §4; mangellisten (Indgangen) |
...
| **BESLUTTET 14/9 — Jonas opretter nøglen; ÅBENT: bygningen. SYNLIGHEDEN LØST #855 (`b60df81d`, 09:48:17 UTC; bevist på skærm 11:50–11:51)** | **CVR-kilden skiftes fra cvrapi.dk til Virk/Erhvervsstyrelsens CVR-API.** cvrapi's kvote hænger på IP (10:37 200 fra Jonas' maskine, 10:40 QUOTA_EXCEEDED fra edge-runtimen, samme CVR; fri igen kl. 12:00 — vinduet er kortere end et døgn). Ingen nøgle findes i secret-listen eller repoet. **Uafhængigt af kilden er en fejlet berigelse nu SYNLIG (#855):** dommen `cvrOpslagMangler` (`src/lib/cvrBerigelse.ts:65`), kvitteringen siger hvad der mangler og hvad rådgiveren skal gøre, signal på virksomhedssiden (kø `stamdata_mangler`), mærke «CVR-opslag mangler» på listen; selvrensende. YKRG APS bærer det (én af 26), Nordic By Hand ikke. | DEL 2 «14. september» §6, §10, §14; mangellisten (Indgangen: kilden åben, synligheden lukket) |
| **LØST 14/9 — #856 (`0f4d8396`, merget 09:56:59 UTC); skal være udrullet før 22/9** | **To mails lovede svar fra noreply — rettet:** `indgangsMail.ts:163` (dag 0) siger nu «så skriv til kontakt@theboardroom.dk — så finder vi ud af det.», og fornyelsens KVITTERING (`fornyelsesMail.ts:200`, `kvitteringMail` — sendes af `stripe-webhook` efter betaling; varsel 1 og 2 bærer ingen adresselinje) siger «så skriv til kontakt@theboardroom.dk.» `KONTAKT_ADRESSE` bor ét sted (`_shared/indgangsMail.ts:67`), re-eksporteret fra `invitationsMail.ts`; kildeværn på alle tre mailfiler. Samme klasse, IKKE rettet: `indgangsMail.ts:184` (dag 14: «er jeg kun en mail væk») og `:207` (dag 25: «så sig til»). | DEL 2 «14. september» §11; mangellisten (lukket) |
...
| **ÅBENT 14/9 — IKKE BYGGET (Jonas: nye events samles i én mail); support-anmodning sendt 14/9, svar afventes** | **Mailloftet er pr. time og pr. workspace (Pro 100 app-mails/time; Business 300; Enterprise 1.000; docs.lovable.dev/features/custom-emails).** Målt 14/9: fire events (08:30, 08:39, 08:45, 08:50) gav 109 mails til 26 adresser på et kvarter; fra 09:10:13 `failed` på seks adresser; Lovable 09:10: «Sending rate limit has been reached». Alle app-mails deler loftet — med 36 medlemmer rammer TRE events det, og den 22. er det en ny ansøgers betalingslink der afvises uden at hun ved hun venter. `publish-event` skriver N rækker i `notifications`; `process-notification-emails` tømmer hvert 5. minut, 50 ad gangen, ældre end 15 min; `email_sent_at` kun ved succes, så køen prøver igen. | DEL 2 «14. september» §13; `recon-event-fanout.md` (uden for repoet); mangellisten (Drift) |
| **UBEVIST I DRIFT — #857 (`588db663`, merget 10:00:35 UTC; fem functions udrullet 10:03)** | **Rate limit-grenen:** 429 klassificeres `rate_limited` (`_shared/mailFejl.ts:45-48`), `email_send_log` får status `rate_limited` (ingen CHECK, ingen migration; `EmailLogView.tsx:66`), dagskvoten tæller den ikke (`send-notification-email:305`), begge løkker stopper ved første. Kan kun bevises af en rigtig 429 — som #815's klokke venter på et fejlet træk. Udrulningen bevist indirekte: `send-report-reminder` tørkørte 12:04, «Period: August 2026 \| Day: 14», ingen exception. | DEL 2 «14. september» §12; mangellisten «Beviser der udestår» |
| **ÅBENT 14/9 — Jonas' opgave: det rigtige CVR-nummer** | **YKRG APS' CVR 44891917 findes ikke i registret** (cvrapi 404 NOT_FOUND, målt ca. 12:00). Hun bærer mærket «CVR-opslag mangler — adresse står tomt» (korrekt), men «kør berigelsen» hjælper hende ikke; berigelsen tjekker kun otte cifre (`_shared/berigelse.ts:88-93`) og brænder ét opslag på hende pr. kørsel. Nyt kort: et NOT_FOUND burde markeres. | DEL 2 «14. september» §14; mangellisten, to kort (Indgangen) |
...
| **LUKKET 14/9 MED FORBEHOLD — #849 (`837ca15a`) rettet i kode, IKKE bevist udrullet (frontend, Update-klik)** | **Fund A: `betalt=1` læses (`Betal.tsx:157`, `laesBetaltHint`); `afgoerKvittering` (`betalKvittering.ts:68-85`) viser «Tak — vi bekræfter din betaling» uden knapper, henter igen hvert 3. sekund i op til 30 sekunder, derefter «Vi mangler den sidste bekræftelse» med «Tjek igen»/«Skriv til os».** Den gamle skærm med tre aktive knapper findes ikke i koden. Beviset er skærmen efter gennemkørslens betaling. | DEL 2 «14. september» §17; mangellisten (Indgangen, lukket) |
...
| **14/9 — PLANEN FREM MOD 22/9** (skrevet 14/9 morgen; ingen kode; DEL 2 «13. september» §26–§27) | **Målinger → gennemkørsel → A/B/C → delingen.** De ni prod-målinger står klar med tærskler og én SQL i `~/Downloads/maalinger-foer-webinaret.md` (tre er secrets og kan kun bevises gennem brug; cron-JOBNAVNE ≠ funktionsnavne; Monday-board-id er en kodekonstant). Gennemkørslen (mangellistens nr. 1) lukker syv kort og de tre «UBEVIST». Tre fund er I STYKKER og har fået kort øverst i mangellisten: A `betalt=1` læses aldrig (rammer hver betalende) — **RETTET #849, i HEAD, lukket 14/9 med forbehold (ikke bevist udrullet; «14. september» §17)**, B invitationens udfald kastes væk (betalt medlem uden login, tavst), C invitationsmailens fire usandheder (i `email_templates`) — **LUKKET #852**. Delingen er et krav (§27): kreativ + upload + 3–4 tekster som tjeklistepunkt for nye; kritisk sti er designet. De atten kort bærer tag «22/9 · nr. N af 18». | DEL 2 «13. september» §25–§27; mangellisten «Rækkefølgen» (tier øverst); `~/Downloads/recon-de-nye-medlemmer.md`, `recon-delingen.md`, `maalinger-foer-webinaret.md` (uden for repoet) |
...
- **Ét mailloft for hele workspacet — og en fan-out pr. event æder det.**
  ...
  se HVORFOR den ikke kom frem. (DEL 2 «14. september» §12–§13)
- **Et gyldigt format er ikke en eksisterende post.** YKRG APS' CVR
  44891917 har otte cifre og findes ikke (404 NOT_FOUND). Berigelsen
  tjekker kun formatet (`harCvr`) og tæller hende som kandidat — ét
  brændt opslag pr. kørsel, for altid, og mærket «kør berigelsen» peger på
  en handling der ikke kan hjælpe. Reglen: et opslag der svarer «findes
  ikke» er en anden tilstand end «ikke slået op endnu» og skal gemmes som
  sådan, så hverken cron eller flade prøver igen. (DEL 2 «14. september»
  §14)
