...
**16. SAMLEMAILENS INTEGRATION — PR 2 (15/9 sent; bygget og testet, IKKE udrullet, ingen PR endnu).**
...
|---|---|---|
| **BYGGET 15/9 sent — PR 2: samlemailens integration i `send-notification-email`; IKKE udrullet; ingen PR endnu** | **Samlemailens typer er ude af køens første hentning og hentes i egen forespørgsel kun i vinduet 17–20 dansk (loft 500); udløbet-reglen på dem; `fordelSamlemail` → `bygSamlemail` → `send()` med label `notification-samlemail` og idempotency-nøgle pr. dansk dato; løkken efter de to eksisterende. Chattens beslutninger 1–6. Værnene ændret bevidst (2 → 3 `skalKoeStoppe`, ordret i §16); nyt kildeværn. Næste: udrulning (functionen OG `_shared/samlemail.ts`), tørkørsel med `nu` kl. 17:30 dansk når der ligger rækker, bevis = første rigtige samlemail senest 21/9 — ellers fallback (præsentationer som info i to uger). Efter udrulningen samles events og opslag til kl. 17. | DEL 2 «15. september» §16, §12; mangellisten (n14-11, w-h) |
