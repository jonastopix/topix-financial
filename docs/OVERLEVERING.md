...
**17. VINDUESMAILENE — PR 3 (15/9 sent; bygget og testet, IKKE udrullet, ingen PR endnu; TEKSTERNE GODKENDT AF JONAS 15/9 — vindue 2 rettet efter hans ord).**
...
- **Migrationen** `20260915230000_fornyelse_vinduesstempler.sql`: `add column if not exists vindue_1_sendt_at timestamptz, vindue_2_sendt_at timestamptz` med kommentar pr. kolonne, husets filhoved (KØRES MANUELT, hvorfor, to navngivne kolonner, fire konsekvenser, bevis-SELECT). IKKE KØRT.
