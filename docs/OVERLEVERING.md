...
| **ÅBENT 14/9 — rammer hver import fra webinaret** | **Importen skriver aldrig `contact_person`.** Rækkebyggeren (`_shared/virksomhedsraekke.ts:171-211`) lægger navnet i `application_context.contact_name`; kolonnen får sin `DEFAULT ''`. Kendt siden 2/9 (`20260902190000:22-27`), aldrig rettet. Kun `monday-webhook` skriver kolonnen — **bevist fra den anden side i gennemkørslen 14/9: Monday-vejen satte `contact_person` «Gennem Kørsel» (§24).** Nordic By Hand sat i hånden 10:14. | DEL 2 «14. september» §4, §24; mangellisten (Indgangen) |
