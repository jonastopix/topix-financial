...
| M12 | `monthly-digest` «Dit {måned}-overblik» | `send-monthly-digest/index.ts:69-330` | cron `send-monthly-digest` 08:00 UTC den 22. (`20260810230000:74`) OG admin-knappen «Send digest» (`EmailTemplatesView.tsx:754-766`); **ingen dag-gate i koden** (grep `getDate`: 0) | alle founders med aktivt medlemskab (`:113-120`), opt-out `digestPrefs` (`:178-182`); springes over når der intet er at sige (`:277`). **Milepæle (8/9, efter #741):** dømt af `_shared/digestMilepaele.ts` → `_shared/milepaelDom.ts` — «Milestones med deadline snart» (frist i dag..+30 dage, højst 3) og for sig «Fristen er passeret på:» (frist passeret 1–60 dage, højst 3, senest passerede først, resten og ældre kun som tal) med udvejen «ret den eller læg milestonen i køleskabet». Før: `progress < 100` og KUN øvre grænse `deadline <= +30`, så april-frister stod som «deadline snart» (21 af 27 hos aktive var overskredne). Samme omgang: `json`-hjælperen var aldrig defineret, så funktionen kastede efter afsendelse | kode | JA — se D4 | kræver prod |
...
| M17b | `fornyelse-kvittering` «Tak — dit medlemskab er fornyet til {dato}» | `_shared/fornyelsesMail.ts` `kvitteringMail` via `stripe-webhook` `sendFornyelseskvittering` | webhook `checkout.session.completed` med `metadata.art = fornyelse` — EFTER perioden og `contract_end_date` er skrevet; idempotent på sessionens id; kaster aldrig | kontoen der betalte (`metadata.user_id`), ellers `contact_email` | kode | JA (bygget 8/9) | kræver prod |
...
| M21 | Rykkerkøen (udkast 18/9): `ansoegning-indkaldelse` · `ansoegning-indkaldt-rykker-1..4` · `ansoegning-samtale-i-morgen` · `ansoegning-samtale-i-dag` · `ansoegning-aftalegrundlag` · `ansoegning-aftalegrundlag-rykker-1..4` (12 skabeloner) | `_shared/ansoegningRykkerMails.ts` (rene byggere); sendt af `ansoegning-rykker-cron/index.ts` | cron `ansoegning-rykker` `*/15 5-15 * * 1-5` UTC via `kald_edge` (`20260918210000`) → forfaldne rækker i `planlagte_haendelser`; trapperne (dag 2/4/7/11 · dagen før + samme morgen · dag 2/5/9/14) planlægges af `_shared/rykkerkoe.ts` ved trinskift | ansøgeren (`ansoegninger.email`) | kode (UDKAST — Jonas godkender teksterne) | NEJ (udkast; migrationerne er IKKE KØRT). Regler: hverdage 07–16 dansk tid, højst én mail pr. person pr. dag, enhver reaktion annullerer resten, «ikke nu» = 3 mdr. pause. Idempotens: `idempotensnoegle` → `message_id` | — |
