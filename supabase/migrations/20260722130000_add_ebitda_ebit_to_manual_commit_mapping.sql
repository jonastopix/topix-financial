-- Fix: EBITDA/EBIT tabes i "Ret data"-flowet (PR fix/ret-data-ebitda-loss).
--
-- Rodårsag: manual-branchens DA→EN CASE i resolve_report_commit_candidate
-- forventede kf-nøglen 'resultat_foer_afskrivninger', men Ret data-formularen
-- gemmer under nøglerne 'ebitda'/'ebit' (fra 2026-07-22 BEREGNET ved gem i
-- saveManualOverride, jf. Mulighed A — afledte tal round-trippes ikke).
-- Uden disse WHEN-grene rammer nøglerne ELSE NULL og droppes, hvorefter
-- commit_report_facts skriver facts UDEN ebitda/ebit (prod-evidens 21-22/7-2026,
-- Topix juni: no-op-gem nedgraderede datasættet).
--
-- Ændring ift. forrige version (20260420190823): PRÆCIS to nye WHEN-grene i
-- manual-CASEn — `WHEN 'ebitda' THEN 'ebitda'` og `WHEN 'ebit' THEN 'ebit'`.
-- Alt andet er uændret kopi. Ingen policies berøres (ingen DROP POLICY).
--
-- Deploy: køres manuelt i Lovable → SQL editor (jf. CLAUDE.md). Verificér efter
-- kørsel med:
--   SELECT pg_get_functiondef('public.resolve_report_commit_candidate(uuid)'::regprocedure);
-- og tjek at CASEn indeholder WHEN 'ebitda' og WHEN 'ebit'.
...
BEGIN
  ...
  IF _out.period_key >= to_char(now(), 'YYYY-MM') THEN
    _out.eligible := false;
