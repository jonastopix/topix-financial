-- Regel 6 i dansk tid (10/9-2026, de-tyve nr. 10 / recon-tallene-tre.md §3a).
--
-- resolve_report_commit_candidate afviste indeværende og fremtidige måneder med
-- to_char(now(), 'YYYY-MM') — databasens UTC. Fladen dømmer «for tidlig» i dansk
-- tid (reportCardView.erForTidligt), så den 1. mellem 00:00 og 02:00 dansk tid
-- (CEST; 00:00–01:00 ved CET) sagde fladen «kan godkendes» og resolveren «ikke
-- afsluttet». Cron'en report-review-cron (samme migration-dag) kører kl. 07 dansk
-- tid og spørger resolveren — den skal svare i samme tid som fladen.
--
-- Ændring ift. forrige version (20260722130000): PRÆCIS én linje — regel 6
-- sammenligner nu mod to_char(now() AT TIME ZONE 'Europe/Copenhagen', 'YYYY-MM').
-- Alt andet er uændret kopi (genereret af den forrige migrations tekst, ikke
-- skrevet af). Ingen policies berøres.
--
-- SKREVET 10/9, IKKE KØRT. Deploy manuelt i Lovable → SQL editor efter merge.
-- Verificér: SELECT pg_get_functiondef('public.resolve_report_commit_candidate(uuid)'::regprocedure);
-- og tjek at regel 6-linjen bærer «AT TIME ZONE 'Europe/Copenhagen'».
-- Grænsetest i prod (kør begge og sammenlign):
--   SELECT to_char(now(), 'YYYY-MM') AS utc, to_char(now() AT TIME ZONE 'Europe/Copenhagen', 'YYYY-MM') AS kbh;
