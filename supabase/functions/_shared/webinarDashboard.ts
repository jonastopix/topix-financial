...
/**
 * Dashboardet SOM SVAR til en ekstern (udkast webinar-deling 21/9): det samme
 * uden `naeste.raekker` — de rå tilmeldinger (mail, navn, by, …), som
 * annoncesporet regnes på. Alt andet i WebinarDashboard er tal, navne på
 * annoncer/kampagner og tekster. Fladen (WebinarVisning) læser denne type, så
 * rådgiverens fulde dom passer (strukturelt) og den delte side får aldrig rækkerne.
 */
export type WebinarDashboardSvar = Omit<WebinarDashboard, "naeste"> & { naeste: Omit<NaesteWebinar, "raekker"> | null };

/** Dashboardet uden de rå rækker — det eneste, der forlader serveren til en ekstern. */
export function udenRaekker(dom: WebinarDashboard): WebinarDashboardSvar {
  if (dom.naeste === null) return { ...dom, naeste: null };
  const { raekker: _raekker, ...naeste } = dom.naeste;
  return { ...dom, naeste };
}
