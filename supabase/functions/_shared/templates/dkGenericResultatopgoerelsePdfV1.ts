/**
 * DK_GENERIC_RESULTATOPGOERELSE_PDF_V1
...
function classifyLineCore(name: string, accountNo: string | null): {
  ...
  if (matchedClasses.length > 1) {
    console.log(`[GenericPDF] AMBIGUOUS label match for "${name}": ${matchedClasses.join(", ")} → unclassified`);
...
function buildSectionMap(lines: PdfParsedLine[]): Map<number, string> {
  ...
  console.log(`[GenericPDF] Section boundaries: ${boundaries.map(b => `${b.sectionCls}@${b.lineIndex}(${lines[b.lineIndex].name})`).join(", ")}`);
...
function detectSignConvention(classified: ClassifiedLine[]): SignConvention {
  ...
  if (revenueAnchors.length === 0) {
    console.log("[GenericPDF] No non-zero revenue anchors for sign detection → UNKNOWN");
    return "UNKNOWN";
  }
  ...
  console.log(`[GenericPDF] Sign anchors: rev(neg=${revNegCount},pos=${revPosCount}), cost(pos=${costPosCount},neg=${costNegCount})`);

  if (revMajNeg && costMajPos) {
    console.log("[GenericPDF] Sign convention: CREDIT (revenue<0, cost>0)");
    return "CREDIT";
  }
  if (revMajPos && costMajNeg) {
    console.log("[GenericPDF] Sign convention: BUSINESS (revenue>0, cost<0)");
    return "BUSINESS";
  }

  console.log("[GenericPDF] Sign convention: UNKNOWN (mixed signs)");
  return "UNKNOWN";
}

// ── Account-Range Anti-Match ──
// Uses shared detector from economicRangeDetector.ts

// ── Template ──

export const dkGenericResultatopgoerelsePdfV1: TemplateEntry = {
  template_id: "DK_GENERIC_RESULTATOPGOERELSE_PDF_V1",
  label: "Generisk dansk Resultatopgørelse PDF",
  ...
  detect(ctx: DetectionContext): number {
    ...
    // Anti-match: yield to specific Dinero template for Dinero-branded documents
    ...
    if (rangeResult.detected) {
      score -= 30;
      console.log(`[GenericPDF] Economic range anti-match: -30 (${rangeResult.method}, ${rangeResult.evidence.join("; ")})`);
    }
  ...
  extract(
    ...
    console.log(`[GenericPDF] Parsed ${lines.length} lines from PDF text`);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (hardOverrideCls) {
        console.log(`[GenericPDF] Hard override: "${line.name}" → ${hardOverrideCls}`);
        classified.push({
      ...
      if (core.cls === "unclassified" && sectionCls) {
        ...
        console.log(`[GenericPDF] Section fallback: "${line.name}" → ${sectionCls}`);
        ...
        finalAmbiguous = true;
        console.log(`[GenericPDF] CONFLICT: label=${core.cls}, section=${sectionCls} for "${line.name}" → ambiguous`);
      }
    ...
    console.log(`[GenericPDF] Classified ${classified.length} detail lines, ${sectionFallbackCount} via section, ${ambiguousCount} ambiguous`);
    ...
    if (ambiguousLines.length > 0) {
      console.log(`[GenericPDF] Remaining ambiguous lines (${ambiguousLines.length}):`);
      ...
      console.log(`[GenericPDF] Ambiguous EBT impact: ${ambiguousImpact.toFixed(2)} (threshold: ${AMBIGUOUS_IMPACT_THRESHOLD})`);
    }
