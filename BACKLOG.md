...
### [P3] Lovable skriver direkte i main uden PR (bogført 2026-08-24)

Lovables bot (gpt-engineer-app) committer direkte til main uden om
PR-flowet. Målt mod git-historikken pr. 2026-08-24: 61 direkte commits
siden 2026-05-01 (PR-flow-æraen), heraf 27 alene i august; det hyppigste
mønster er parret "Work in progress" efterfulgt af "Lovable update"
(19 commits), resten mest "Changes".

KONKRET EKSEMPEL 2026-08-24: en anmodning i build-chat om at DEPLOYE
edge-funktionen generate-budget-scenarios resulterede i commit ae3cb6bb +
merge 6bf6edde, der ændrede AUTENTIFICERINGSLAGET:

- `src/integrations/supabase/client.ts` — `auth.storage` gik fra
  `localStorage` til `brokeredPreviewStorage()`.
- `src/integrations/supabase/previewAuthStorage.ts` — ny fil, 88 linjer,
  deler login-session med Lovable-editoren via postMessage på
  preview-domæner.

Commit-beskeden var deploy-anmodningens tekst. Der er ingen sammenhæng
mellem det der blev bedt om og det der skete.

VURDERET UFARLIG PÅ PROD (kode-verificeret mod 6bf6edde): funktionen
returnerer `localStorage` når værten ikke er en Lovable preview-zone
ELLER siden ikke er i en iframe. `app.theboardroom.dk` opfylder ingen af
delene, så medlemmernes login er urørt. Modtager-origin valideres desuden
mod Lovables egne domæner (`lovable.dev`/`gptengineer.app`-regex), og
projekt-id læses kun fra positioner i værtsnavnet en bruger ikke kan
styre.

Begge filer bærer "This file is automatically generated. Do not edit it
directly" — det er platformskode, ikke noget vi har bedt om. Fjernes den,
kommer den formentlig igen.

KONSEKVENS AT KENDE: main er ikke under vores kontrol alene. En branch
kan være bygget oven på kode ingen har læst, og CI kører ikke som pull
request på disse commits (kun push-triggeren på main fanger dem — derfor
kører test.yml og edge-function-auth.yml bevidst på BEGGE triggere).
Tjek `git log --author="gpt-engineer-app"` når noget uventet dukker op i
historikken.

Placeret ved siden af Update-knap/merge≠deploy-punktet ovenfor — de to
hører sammen: Lovable ejer både deploy-kanalen og en skrivekanal til main.
...
### [P3] Mobilgennemgang (oprettet 2026-08-26)

Samlepunkt for kendte mobil-/touch-mangler til en samlet gennemgang:
- Estimat-mærkets forklaring bæres af title-attributten og er derfor utilgængelig på touch. Nøgletal har forklaringen som brødtekst, men forsidens tal-strip har kun title. (estimat-markering, 2026-08-26)
