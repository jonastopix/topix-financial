              ...
              <div key={inv.id} className="px-4 py-2.5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {/* «Sendt» kun når email_send_log bærer en afsendt invitationsmail
                        (lastSentAt, Members.tsx #701). Uden spor har vi kun rækkens
                        oprettelse — så hedder det «Oprettet», ikke «Sendt» (rettet 7/9,
                        recon-virksomhedssidens-aftale.md §7a). */}
                    <p className="text-xs text-muted-foreground">
                      {inv.companyName} ·{" "}
                      {inv.lastSentAt
                        ? `Sendt ${format(new Date(inv.lastSentAt), "d. MMM yyyy", { locale: da })}`
                        : `Oprettet ${format(new Date(inv.created_at), "d. MMM yyyy", { locale: da })}`}
                    </p>
                  </div>
                </div>
