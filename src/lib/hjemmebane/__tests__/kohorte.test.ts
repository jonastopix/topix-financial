...
describe("kohorteLinje — hvem er med", () => {
  it("nul nye → m 0, n 0, ingen navne", () => {
    expect(koer({ virksomheder: [v("a")], medlemmer: [m("a", "u1", "2026-06-01T10:00:00Z")] })).toEqual({ m: 0, n: 0, ikkeKommetIgen: [], udeladtIDag: 0 });
  });
  ...
  it("én kom igen: login på en senere dansk dato", () => {
    ...
    expect(r).toEqual({ m: 1, n: 1, ikkeKommetIgen: [], udeladtIDag: 0 });
  });
  ...
  it("én ikke kommet igen: kun startdagens logins", () => {
    ...
    expect(r).toEqual({ m: 1, n: 0, ikkeKommetIgen: ["Firma a"], udeladtIDag: 0 });
  });
  ...
  it("virksomhed uden medlemmer er udeladt — ingen start", () => {
    expect(koer({ virksomheder: [v("a")], medlemmer: [] })).toEqual({ m: 0, n: 0, ikkeKommetIgen: [], udeladtIDag: 0 });
  });
  ...
  it("startet i dag (dansk dato) er ikke i M men tælles i udeladtIDag", () => {
    ...
    expect(r).toEqual({ m: 1, n: 0, ikkeKommetIgen: ["Firma b"], udeladtIDag: 1 });
  });
  ...
  it("ugyldige datoer ignoreres roligt", () => {
    ...
    expect(r).toEqual({ m: 0, n: 0, ikkeKommetIgen: [], udeladtIDag: 0 });
  });
});
