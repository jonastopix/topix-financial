/**
 * src/lib/ansoegning/skema.ts
 *
 * Ansøgningsformularens SKEMA og dens rene domme — spejlet ordret i
 * supabase/functions/_shared/ansoegningSkema.ts (paritetstest:
 * src/lib/__tests__/ansoegningSkemaParitet.test.ts, samme mønster som
 * omkostningsnoegler). Kroppen under filhovedet SKAL være byte-ens i de
 * to filer; kun dette hoved må afvige.
 *
 * Ingen React, ingen Supabase, ingen Deno. Fladen (/ansoeg) og
 * edge-funktionen ansoegning-gem dømmer det samme svar med den samme
 * kode, så et svar der slipper igennem på telefonen aldrig afvises
 * serverside — og omvendt.
 *
 * Udkast 18/9-2026 (~/Downloads/udkast-ansoegning-formular). Feltnavnene
 * er FORSLAG indtil A's datamodel (udkast-ansoegning-motor) er læst —
 * se README §3.
 */
