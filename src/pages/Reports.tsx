...
const Reports = () => {
  ...
  // Build program-centric month slots
  const { historicMonths, programMonths, programDelivered } = useMemo(() => {
    if (!programStart) {
      // Fallback: show all months as "program"
      const allKeys = Object.keys(reportsByMonth).sort();
      const slots = allKeys.map(key => {
        const [y, m] = key.split("-");
        return { key, month: parseInt(m, 10) - 1, year: y, report: reportsByMonth[key] };
      });
      return { historicMonths: [] as typeof slots, programMonths: slots, programDelivered: slots.filter(s => s.report?.status === "processed").length };
    }

    const startYear = programStart.getFullYear();
    const startMonth = programStart.getMonth(); // 0-indexed

    // Generate the 12 program months
    const progMonths: { key: string; month: number; year: string; report?: DbReport }[] = [];
    for (let i = 0; i < 12; i++) {
  ...
  return (
    <AppLayout>
      ...
      {(programMonths.length > 0 || historicMonths.length > 0) && (
