...
const Feedback = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Fetch reports with AI analysis
      const { data: reports } = await supabase
        .from("financial_reports")
        .select("id, report_period, ai_analysis, uploaded_at")
        .eq("user_id", user.id)
        .not("ai_analysis", "is", null)
        .order("uploaded_at", { ascending: false });

      // Fetch milestones to determine status
      ...
      (reports || []).forEach((report) => {
        ...
        const reportLabel = report.report_period || "Ukendt periode";

        analysis.key_findings.forEach((finding: any, idx: number) => {
          // Match to milestone for status
          ...
          if (matchedMilestone) {
            if (matchedMilestone.progress >= 100 || matchedMilestone.status === "completed") {
              status = "actioned";
            } else if (matchedMilestone.progress > 0) {
              status = "improved";
            }
          }
          // Positive findings without milestones are "improved" by nature
          if (!matchedMilestone && finding.severity === "positiv") {
            status = "improved";
          }
  ...
  const progressPct = progressItems.length > 0 ? Math.round((actionedCount / progressItems.length) * 100) : 0;
    ...
    <AppLayout>
      ...
      {loading ? (
        <div className="glass-card rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Henter anbefalinger...</p>
        </div>
      ) : progressItems.length === 0 ? (
        <div className="glass-card rounded-xl p-12 text-center">
          <Sparkles className="h-10 w-10 text-muted-foreground/30 mx-auto mb-4" />
          <p className="text-sm text-foreground font-medium mb-1">Ingen AI-anbefalinger endnu</p>
          <p className="text-xs text-muted-foreground">Upload en rapport og generer en AI-analyse for at se anbefalinger her.</p>
        </div>
      ) : (
        <>
          {/* Scoreboard */}
